...
(async()=>{
  ...
  const output=path.resolve(root,process.env.MQ_PDF_AUDIT_OUTPUT_DIR || 'validation_artifacts/pdf_accessibility/public_sync_v1');
  assert.ok(output.startsWith(path.join(root,'validation_artifacts/pdf_accessibility')+path.sep));
  fs.mkdirSync(output,{recursive:true});
  fs.writeFileSync(path.join(output,'routing.json'),JSON.stringify({localRouting:'PASS',count:records.length,records,unsafePathsRejected:true,missingManifestFallback:'null; no invented PDF URL',liveSiteChecked:false,deployed:false},null,2)+'\n');
