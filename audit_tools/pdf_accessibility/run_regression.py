...
if phase not in ('baseline','final','pilot_blockers_v1','full_batch_v1_baseline','full_batch_v1_general','full_batch_v1_micro','full_batch_v1_macro','full_batch_v1_final'):
...
write_json(out/'results.json',{'task':'PDF_ACCESSIBILITY_FULL_BATCH_V1' if phase.startswith('full_batch_v1') else TASK,'phase':phase,'total':len(results),'passed':sum(r['passed'] for r in results),'results':results,'temporaryDirectory':str(scratch)})
