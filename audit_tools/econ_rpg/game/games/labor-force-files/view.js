...
import { summaryChart } from './summary-chart.js';
import { fieldFeedback, fieldSummary } from './field-feedback.js';
...
export function board(run) {
  ...
  return `<figure class="scene"><img src="../../art/scenes/labor-force-files/labor-${m.scene.id}.webp" width="1672" height="941" alt="${esc(m.scene.alt)}"></figure>
...
function numeric(run,target) {
  ...
  const fields=Object.fromEntries(fieldFeedback(run).map(f=>[f.key,f]));
  const status=key=>fields[key]?` aria-invalid="${!fields[key].correct}" data-valid="${fields[key].correct}"`:'';
  const message=key=>`<p id="field-${key}-feedback" class="field-feedback ${fields[key]?(fields[key].correct?'correct':'recheck'):''}">${fields[key]?esc(fields[key].text):''}</p>`;
  return `<form id="answer-form">${Object.keys(target).map(key=>key==='participation'?`<fieldset class="participation-field"><legend>What happens to LFPR?</legend>${[['rises','Rises'],['falls','Falls'],['unchanged','Stays unchanged']].map(([id,label])=>`<label class="radio"><input type="radio" name="participation" value="${id}" required aria-describedby="field-participation-feedback"${status(key)} ${run.solved?'disabled':''} ${run.answer?.participation===id?'checked':''}>${label}</label>`).join('')}${message(key)}</fieldset>`:`<div class="field"><label for="answer-${key}">${labels[key]}</label><input id="answer-${key}" name="${key}" type="text" inputmode="decimal" autocomplete="off" required aria-describedby="number-help field-${key}-feedback"${status(key)} ${run.solved?'disabled':''} value="${esc(run.answer?.[key]??'')}">${message(key)}</div>`).join('')}<p id="number-help" class="help">Counts are whole people. Round rates to one decimal; a trailing % is optional.</p><button type="submit" class="primary" ${run.solved?'disabled':''}>CHECK REPORT</button></form>`;
}
