...
- `loan-target.js`: UI adapter deriving new lending from the entered ending loan total; the economic model is unchanged.
- `loan-target.test.js`: ending-total validation and equivalence tests.
...
```sh
node --test audit_tools/econ_rpg/game/games/money-in-motion/model.test.js audit_tools/econ_rpg/game/games/money-in-motion/loan-target.test.js
...
The subtitle and permanent legend were removed. About this model and Banking Terms are collapsed references beside the title; two-stage hints are available on request at the lending desk. Accounting panels use light ledger surfaces and dark numbers. Act 2 uses shorter bank cards, a single totals strip, a labeled active bank, and ending-total inputs matching Act 1. All three active-bank tasks fit with the network and totals at 1280×720 and 1440×900 with disclosures closed. Smaller layouts reflow to two-column or stacked bank cards. The economic model, multiplier/scenario logic, and instructional close remain unchanged.
