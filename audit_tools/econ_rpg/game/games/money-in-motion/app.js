...
function balance(b,showDerived=true){
  ...
  return `<section class="panel" aria-label="${b.name} balance sheet"><p class="eyebrow">Balance sheet</p><h3>${b.name}</h3>
    <div class="balance-grid"><div><h4>Assets</h4>${row('▣ Reserves',b.reserves,'reserves')}${row(showDerived?'↗ Loans':'↗ Existing loans',b.loans,'loans')}${row('Total assets',roundMoney(b.reserves+b.loans),'total')}</div><div><h4>Liabilities</h4>${row('▤ Deposits',b.deposits,'deposits')}${row('Total liabilities',b.deposits,'total')}</div></div>
    <p class="balanced">✓ Balance sheet balanced</p><div class="reserve-desk"><h4>Reserve account · ${pct(state.reserveRatio)} requirement</h4><dl><div><dt>Total reserves</dt><dd>${dollars(b.reserves)}</dd></div><div><dt>Required reserves</dt><dd>${showDerived?dollars(required):'Awaiting your entry'}</dd></div><div><dt>Excess available for lending</dt><dd>${showDerived?dollars(excess):'Calculate at the loan desk'}</dd></div></dl></div></section>`;
}
function loanForm(mode){
  const b=mode==='training'?state.currentBank:state.bankNetwork[state.networkStep];
  return `<form id="loan-form" data-mode="${mode}" novalidate><div class="mobile-snapshot" aria-label="Current bank balances">${row('▤ Total deposits',b.deposits,'deposits')}${row('▣ Total reserves',b.reserves,'reserves')}${row('↗ Existing loans',b.loans,'loans')}</div><div class="fields">${field('required',mode==='training'?'Required reserves after transactions ($)':'Required reserves ($)')}${field('loan',mode==='training'?'Total loans after this round ($)':'Total loans after this bank lends ($)')}</div><button class="primary" type="submit">Authorize loan &amp; clear payment</button><div class="entry-support"><p class="small" id="entry-help">Enter ending totals in dollars, without commas.</p><details class="hint"><summary>Need a hint?</summary><p>Required reserves are based on the bank’s total deposits, not just today’s new deposit.</p><details><summary>Show the next hint</summary><p>Apply the ${pct(state.reserveRatio)} reserve requirement to total deposits. Reserves above that minimum can support new lending. Add that new lending to existing loans to find the ending loan total.</p></details></details></div>${feedback()}</form>`;
}
function training(){
  ...
  <div class="desk"><div class="panel"><div class="slip"><h3>Transaction queue</h3><ul>${events.map(e=>`<li>${e.type==='deposit'?'Customer deposit +':'Customer withdrawal −'}${dollars(e.amount)}</li>`).join('')}</ul><span class="small">${state.posted?'✓ Posted to deposits and reserves':'Ready to post'}</span></div>
    ${!state.posted?`<p>${state.round===0?'Post the deposit to open the bank’s accounts.':state.round===2?'Post both events to update deposits and reserves.':'Post the new deposit. Existing loans stay on the books.'}</p>${button('post','Post queued transactions')}`:
    !state.cleared?`<h3>Loan desk</h3><p>Use the bank’s full balance sheet to determine its required reserves and ending loan total.</p>${loanForm('training')}`:
    `<div class="confirmation"><h3>Payment cleared</h3><p class="new-loan"><strong>New loan authorized: ${dollars(last.amount)}</strong></p><p>${last.amount?`The borrower spent ${dollars(last.amount)} outside this bank. Common Ground exchanged ${dollars(last.amount)} in reserves for a ${dollars(last.amount)} loan asset.`:'You kept the excess reserves. No loan or outgoing payment was created.'}</p><p class="small">${dollars(maxLoan(state.currentBank,state.reserveRatio))} remains available for lending.</p></div>${state.round<2?button('next-round','Open next transaction queue'):`<p>You have been running one bank. But where did the money from those loans go?</p>${button('network','Pull back to the banking network')}`}`}
...
function network(){
  ...
  return `${heading('Act 2 · Across Town · Bank network','Follow the payment')}
    <p class="network-intro">Follow the same process across four banks. Each loan is spent and becomes a deposit at the next bank.</p>
    ${ledger()}${bankCards()}
    <div class="panel network-task">${last?`<div class="transfer"><span class="transfer-symbol" aria-hidden="true">▣ →</span> <strong>${dollars(last.amount)} settled:</strong> ${BANK_NAMES[last.from]} → ${BANK_NAMES[last.to]}</div>`:`<p class="network-origin">Clean system: ${dollars(state.initialReserveInjection)} of new reserves supports the opening deposit.</p>`}${step<3?`<h3>${BANK_NAMES[step]} · ${pct(state.reserveRatio)} requirement · lend all excess</h3>${loanForm('network')}`:
...
work.addEventListener('submit',e=>{
  ...
  const target=number('loan');if(target===null)return;
  const b=state.gamePhase===1?state.currentBank:state.bankNetwork[state.networkStep],minimum=requiredReserves(b.deposits,state.reserveRatio),plan=loanTarget(b,state.reserveRatio,target);
  const loan=plan.newLoan;
  state.studentHistory.push({type:'proposed-loan',phase:state.gamePhase,round:state.round,totalLoansTarget:target,amount:loan,required});
  if(required!==minimum){error(`The reserve requirement applies to all ${dollars(b.deposits)} of deposits. At ${pct(state.reserveRatio)}, required reserves are ${dollars(minimum)}.`, 'required');return;}
  if(target<plan.existingLoans){error(`The bank already has ${dollars(plan.existingLoans)} in loans. The ending total cannot be lower; no loan repayments are queued. Enter at least ${dollars(plan.existingLoans)} to keep existing loans unchanged.`, 'loan');return;}
  if(!plan.withinRange){error(`You can finish this round with at most ${dollars(plan.maximumTotalLoans)} in total loans. The bank already has ${dollars(plan.existingLoans)} in loans, and its current excess reserves allow ${dollars(plan.maximumNewLoan)} of new lending.`, 'loan');return;}
  if(state.gamePhase===2 && target!==plan.maximumTotalLoans){error(`This guided chain tests full lending: finish with ${dollars(plan.maximumTotalLoans)} in total loans, including ${dollars(plan.maximumNewLoan)} of new lending. You will test holding a buffer in Act 4.`, 'loan');return;}
