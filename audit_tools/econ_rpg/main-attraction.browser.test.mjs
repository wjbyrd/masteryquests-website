...
import { coverage } from './main-attraction-qa.mjs';
const { chromium } = createRequire(import.meta.url)(process.env.PLAYWRIGHT_MODULE || 'playwright');
const out = fileURLToPath(new URL('../../tmp/econ-rpg/main-attraction/', import.meta.url));
await mkdir(out, { recursive: true });
const server = previewServer(); await new Promise(resolve => server.listen(0, '127.0.0.1', resolve));
const origin = `http://127.0.0.1:${server.address().port}`, url = `${origin}/?scenario=main-attraction`;
const key = storageKey(scenario), homeKey = storageKey(housing), errors = [], external = [], violations = [], passed = [], images = new Set();
...
try {
  ...
  const housingBytes = () => page.evaluate(k => localStorage.getItem(k), homeKey);
  ...
  const panel = async () => {
    ...
    assert.equal(await page.locator('#state-title').innerText(), 'Park conditions');
  ...
  await page.keyboard.press('Tab'); assert.equal(await page.evaluate(() => document.activeElement.dataset.choice), 'raise');
  assert.equal(await page.evaluate(() => getComputedStyle(document.activeElement).outlineStyle), 'solid');
  await page.keyboard.press('Space'); await focus(); assert.match(await page.locator('#announcement').innerText(), /Park earnings increased/);
  ...
  assert.equal(await housingBytes(), originalHousing);
  await choose('lower'); await next(); await choose('reserve'); await next(); await choose('single'); await next(); await choose('overhaul'); await next();
  assert.equal(await page.locator('[data-choice="expand"]').count(), 0);
  const gated = await saved(); await page.reload(); await page.getByRole('button', { name: /Resume decision/ }).click();
  assert.deepEqual(await saved(), gated); assert.equal(await page.locator('[data-choice="expand"]').count(), 0);
  await choose('throughput'); await next(); assert.equal(await page.locator('[data-choice="differentiate"]').count(), 0);
  ...
  await blocked.goto(url); await blocked.getByRole('button', { name: 'Begin scenario' }).click(); await blocked.locator('[data-choice="lower"]').click();
  assert.match(await blocked.locator('#storage-notice').innerText(), /could not be saved/);
  assert.equal(await blocked.locator('#view-title').innerText(), 'Operating consequences'); await blocked.close();
  ...
  for (const v of scenario.sceneSet.variants) {
    ...
    assert.equal((await context.request.get(`${origin}/art/source/main-attraction/main-attraction-${v.id}.png`)).status(), 404);
  }
  const allowedImages = new Set([...scenario.sceneSet.variants, ...housing.sceneSet.variants].map(v => v.src.slice(1)));
  for (const image of images) assert.ok(allowedImages.has(image));
  assert.equal([...images].filter(p => p.includes('/main-attraction/')).length, 6);
  violations.push(...await page.evaluate(() => window.__violations));
