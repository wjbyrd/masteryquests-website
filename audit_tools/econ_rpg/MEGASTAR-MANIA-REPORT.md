...
Completed September 20, 2026. **Ready for instructor QA.** Development only; nothing deployed, published or pushed.

Play at <http://127.0.0.1:4179/?scenario=megastar-mania>. The title is exactly **Megastar Mania**, without a subtitle. Start with routes 1–6 in [MEGASTAR-MANIA-QA-PATHS.md](MEGASTAR-MANIA-QA-PATHS.md); together they show all seven scenes and all six endings. Sixteen routes cover every reachable conditional consequence and choice.
...
- `megastar-mania.test.mjs`: eight suites for structure, economic invariants, scenes, endings, persistence, assets and earlier scenario behavior.
...
| A Smaller Circuit | 30 | A compact schedule serves the remaining buyers, sometimes with open seats |

“A Smaller Circuit” refines the suggested “Niche but Loyal” title: a compact route can retain some core listeners while goodwill still needs repair. Endings are not ranked. No route reaches the maximum of all five indicators, and there is no aggregate score or prescribed best ending.
...
| expanded-tour | Recent dates/venue expansion with enough purchases for the depicted crowd; tour map is the primary signal |
...
Priority is supply shock, demand drop, demand boom, supported expansion, shortage, surplus, baseline. It uses history and market conditions, not a single raw score. An overexpanded tour uses empty-seat imagery rather than a misleading crowded map scene. A demand decline that still leaves a shortage retains shortage imagery. A crossover that cannot fill seats retains demand-drop imagery.
...
**Automated tests: 37 passed, zero failed.** This includes the eight new Megastar tests and all existing engine, scene, park, PPF and publication tests.

The exhaustive new checks validate unique IDs, targets, all reachable nodes/choices/outcomes, bounds, six decisions on all 729 routes, every ending and every scene. All 9,477 decision/consequence/ending phases replay exactly. Four save keys remain isolated. Restart/replay identity, corrupted state and version mismatches are checked. Economic assertions cover price versus demand, supply-only changes, publicity-only changes, revenue under binding capacity, resale-only shortages, recent cancellation scenes and market-consistent endings.
...
**Browser checks: passed in headless Chrome.** Megastar completed 16 coverage routes at each of 1280px, 390px and 320px: **48 full playthroughs**, all seven scenes and all six endings at each width. Existing browser suites also passed: 41 housing runs, 27 park runs and 33 PPF runs, for **149 completed runs across the four suites**, plus focused checks.
...
Evidence: `tmp/econ-rpg/megastar-mania/browser-results.json`, screenshots in that folder, `tmp/econ-rpg/megastar-units.log`, and the three `megastar-*-regression.log` files.
...
Use [routes 1–6](MEGASTAR-MANIA-QA-PATHS.md) first, in order: early surplus and reputational weakness; balanced opening and successful capacity fit; low-price shortage and resale; expansion followed by crossover; excess capacity; a compact tour. Routes 7–16 cover the remaining conditional economic consequences. In particular, compare a demand decline with and without spare capacity, and stronger crossover demand with already-sold-out seats.
