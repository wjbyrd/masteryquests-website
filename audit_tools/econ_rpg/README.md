...
The complete private [Games library](http://127.0.0.1:4179/games/) links all nine mini-games. Each game has a permanent **RETURN TO GAMES** link. See the [mini-game accessibility audit](MINI-GAME-ACCESSIBILITY-AUDIT.md) for the original seven-game review and the [CPI LIVE report](CPI-LIVE-REPORT.md) for the new fixed-basket price-index game.
...
- [Room to Stay](http://127.0.0.1:4179/?scenario=housing-crisis): binding rent ceiling and housing markets; the default scenario.
...
- [Labor Force Files](http://127.0.0.1:4179/games/labor-force-files/): classify workers, calculate UR and LFPR, trace population flows and audit a headline; [implementation and QA report](LABOR-FORCE-FILES-REPORT.md).
...
The learner is Linden’s housing advisor after the city has already imposed a binding rent ceiling below equilibrium. Six decisions over two years manage its shortage, maintenance, supply and distributional consequences. The experience assesses applied decisions, causal reasoning, distributional tradeoffs, second-order effects and path dependence. It does not grade retrieval, classify concepts or award an answer score. The loop is decision → consequence → visible state changes → next decision → ending → debrief → formal model and transfer applications.
...
Six decisions use seven authored nodes: vacancy allocation → maintenance → long-run supply → incumbent/newcomer distribution → mitigation → two-year review. The opening new-lease exemption leads to a distinct distribution branch. The alternative lottery preserves controlled new leases. Both reconverge at assistance layered on the ceiling. New construction can be exempted separately only if new leases remain controlled; its supply response still has a lag. Existing inspections, repairs, subsidies, permitting reform, temporary aid, tapering and final priorities retain their tradeoffs.
...
Current exhaustive QA: **252 paths**, six decisions each, seven nodes, 18 node/choice pairs, five reachable endings, no dead ends, no orphans and no cycles. Ending counts: fiscal 75, shortage 33, supply 8, protected 88, mixed 48. See [QA-PATHS.md](QA-PATHS.md) for consequence/choice/ending coverage. Scenario version 2 resumes current saves exactly and safely rejects version-1 saves rather than replaying an obsolete policy path. The initial state is relief 5, availability 2, quality 5, budget 7, construction incentive 2; maintenance pressure is prospective, not instant damage.

The final price-ceiling diagram depicts the initial controlled market, not estimated Linden quantities or the final market after exemptions. Three applications cover the shortage under a lottery, a tighter ceiling in another city and additionality. Housing additionality and Main Attraction pricing each use four numeric variants, selected from the existing random run ID; choices and economic states remain deterministic. See the [instructional follow-up audit](INSTRUCTIONAL-FOLLOWUP-AUDIT.md).
...
Independent keys: `mq.econ-rpg.housing-crisis`, `mq.econ-rpg.main-attraction` and `mq.econ-rpg.ppf`; Room to Stay uses version 2; the other two use version 1. Routing between them preserves their separate saves. A save includes scenario ID/version, a random local run ID, local start time, phase, current node, final ending if any, indicator state and the decision history. The random run ID identifies only this local run; it is not a fingerprint or stable user identifier. There is no personal data input.
