...
const out='tmp/econ-rpg/cpi-live';await mkdir(out,{recursive:true});
...
for(let seed=0;coverage.size<19&&seed<10000;seed++){
...
async function inspect(page,label){
  ...
  assert.deepEqual(await page.locator('.cpi-receipt tbody th small').allTextContents(),CONFIG.basket.map(b=>`${b.quantity} × fixed`));
  for(const button of await page.locator('button,input,.return-games').all())assert.ok((await button.boundingBox()).height>=44,label+' touch target');
...
async function play(context,seed,label,capture,wrong=true){
  ...
  for(const phase of engine.PHASES){
    ...
    if(wrong){
      const bad=numeric?Object.fromEntries(Object.keys(target).map(k=>[k,'banana'])):phase==='meaning'?'rate':phase==='weight'?(target==='a'?'b':'a'):phase==='audit'?CONFIG.audits.find(a=>a.id!==target).id:phase==='timeline_compare'?(target==='stable'?'deflation':'stable'):'below';
      if(numeric){for(const k of Object.keys(target))await fill(page,k,'banana');await press(page,'button[type=submit]');}
    ...
    if(phase==='timeline_rate')assert.equal(await page.locator('.timeline-table tbody tr').count(),3);
  ...
  await press(page,'[data-action=replay]');assert.equal(await page.locator('#basket-total').innerText(),'To calculate');
