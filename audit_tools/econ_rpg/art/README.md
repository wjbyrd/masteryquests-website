...
All fourteen files are preserved at 1448 × 1086. The PNGs were already named `megastar-*.png` in `game/art/sources/`; they moved to the canonical private `art/source/megastar-mania/` folder. The supplied WebPs stayed in `game/art/scenes/megastar-mania/`. On September 21 the user supplied an updated `expanded-tour.webp` at that same runtime path. It is used without modification; the PNG master remains the earlier supplied version. The manifest records the updated WebP hash and retains its prior hash for provenance. No generation, conversion, recompression, cropping or pixel editing was performed by Codex. Original locations, current supplied-file byte counts, dimensions and SHA-256 hashes are recorded in `megastar-mania-assets.json` and enforced by tests.
...
## Gameday Rivals — supplied scenes and fixed delivery overlays

Six supplied 1448 × 1086 WebPs remain unchanged under `game/art/scenes/gameday-rivals/`: home opener, small-team game, conference game, heated rival, homecoming and biggest rival at night. Each round selects its own exact file. All use the same uncropped 4:3 image frame; only the sixth selects the night asset. The six corresponding supplied PNG masters moved unchanged from `game/art/sources/` to `art/source/gameday-rivals/`, outside the preview root. Original paths, byte counts, dimensions and SHA-256 hashes for all twelve files are in `gameday-rivals-assets.json`. No image was regenerated, edited, converted or recompressed.

**Missing reference:** `canonical-day.webp` was not found in the user-confirmed runtime folder or either project folder. It is not replaced by a round image. Geometry comparisons against that canonical source remain pending; the six actual round scenes are available and directly inspected.

Background crowds and traffic represent market opportunity, including any colored delivery vehicles already in the supplied artwork. Only the outlined SVG P/orange and R/green vehicles represent this round’s revealed promotion choices. Standard gets two units per firm; Aggressive gets four. The visible caption reports each firm's count, and the SVG has a textual alternative. Current activity remains unrevealed before the player's commitment.

Eight authored road slots are shared across the six images. Their coordinates, lane, direction, orientation and vehicle type are in `game/scenarios/gameday-rivals-scenes.js`; the renderer does not invent positions or animate travel. Northwest vehicles face 201°, southeast vehicles 21°. The full scaled footprints are checked against road/lane polygons and crosswalk exclusions. Cars and moped delivery boxes use their firm's single color and P/R mark. Browser evidence covers every one of the 24 round/activity combinations at desktop, 390px and 320px. The vehicles are schematic overlays, not literal reconstructions of the underlying traffic. See [implementation and QA report](../GAMEDAY-RIVALS-REPORT.md).
