...
function guide({summary, selected}) {
  ...
  lines.push('', 'D = underlying Fan Interest; S = available ticket capacity; R = financial strength from receipts after refunds; G = goodwill; M = career momentum. The numbers in this instructor table are internal ordinal state, not empirical estimates. The student panel shows qualitative Fan Interest and Ticket Supply labels instead of their numbers, plus a separate Ticket Market gauge derived at the posted price. Do not subtract D and S to diagnose shortage.', '', '## Choice key', '');
