...
function guide({summary, selected}) {
  const lines = ['# Megastar Mania — instructor QA paths', '',
    ...
    `${summary.paths} legal paths each take six decisions. These ${selected.length} routes cover every reachable node, choice and conditional consequence, all seven scenes and all six endings. Read routes 1–6 first; the remaining rows complete economic boundary coverage.`, '',
  ...
  lines.push('## Focused review', '',
    ...
    '- Added dates and bigger venues change supply only. If there are no extra willing buyers at the price, new seats do not raise ticket receipts. The expanded-tour map appears for an expanded schedule with strong attendance; an overexpanded schedule uses empty-seat art.',
