...
test('staging uses the existing explicit deny rule; current production build excludes all prototype files', () => {
  ...
  const supplied = ['approved-assets.json','main-attraction-assets.json','ppf-assets.json','megastar-mania-assets.json'].flatMap(name => JSON.parse(readFileSync(new URL(`./art/${name}`, import.meta.url), 'utf8')));
  const artHashes = new Set(supplied.map(a => a.sha256));
  for (const file of files) {
    assert.doesNotMatch(file, /econ[_-]rpg|housing-crisis/i);
    if (/\.(html|js|json|css)$/i.test(file)) assert.doesNotMatch(readFileSync(file, 'utf8'), /mq\.econ-rpg|scenarios\/housing-crisis|Room to Stay|The Main Attraction|main-attraction\.js|The Economy[’']s Edge|scenarios\/ppf|Megastar Mania|megastar-mania|Jules Arlen/);
