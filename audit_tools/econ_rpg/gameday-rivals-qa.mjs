...
export function manualGuide(data) {
  ...
  lines.push('','## All player histories','',...histories.map((s,i)=>`${i+1}. \`${notation(s)}\``),'','## Classification reachability','',...Object.entries(data.summary.classifications).map(([id,count])=>`- ${CLASSIFICATIONS.find(c=>c.id===id).title}: ${count} seasons`),'',
    ...
    '- At desktop widths of 1200px or more, the uncropped image occupies 54% of the round body and decisions/reveal reuse the 46% right column. At 1366×768 and 1280×800, check both actions at scroll position zero and the history heading after reveal. Below 1200px, image, controls/reveal, desk and history stack; the tablet scene is capped at 520px and phone framing stays wide.',
    '- Scroll across the Season History boundary with desk help both open and closed: the non-sticky desk stays in its own row with a clear gap. The completed debrief must restore its original spacious presentation, with no active-round columns or compact styling.',
    ...
    'Patch validation: 2,048 complete seeded seasons, 12,288 reveals; all round splits/weights, reproducible complementary season shares, both differently weighted win cases and unchanged profit/rival/classification paths are asserted. Latest September 22 density pass: 54/54 unit, regression and publication tests passed, plus 58 complete browser seasons (42 coverage seasons and 16 additional all-Standard/all-Aggressive density seasons across eight viewports). Desktop actions, continuation and history headings fit without scrolling; narrow screens stack. All nine-width disclosure/scroll-boundary checks passed. The 27 layout fixtures cover intro, reveal and debrief, with help closed/open/closed again at four scroll positions. Activity bars, image hashes and the original final-debrief screenshot remain intact; no overlap, page overflow or runtime error remains. See GAMEDAY-RIVALS-REPORT.md for evidence.', '',
