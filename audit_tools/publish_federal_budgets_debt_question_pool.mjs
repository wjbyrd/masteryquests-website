...
function render(){
  ...
  const reviewManifest=JSON.parse(fs.readFileSync(reviewManifestPath,"utf8")),reviewSource=JSON.parse(fs.readFileSync(reviewSourcePath,"utf8"));reviewManifest.concepts=reviewManifest.concepts.filter(item=>item.canonicalConceptId!==author.CONCEPT_ID);reviewManifest.concepts.push({canonicalConceptId:author.CONCEPT_ID,displayName:TITLE,discipline:"macro",areas:["macro"],parentId:null,childIds:[],selectable:true,selectableCard:true,diagnosable:true,selectionRole:"standalone",disposition:"NO_SHEET_INTEGRATION_META",reason:"This independently diagnosable production concept was added without an immutable faculty review PDF."});reviewManifest.summary.canonicalConceptCount=library.conceptCount;reviewManifest.summary.dispositionCounts.NO_SHEET_INTEGRATION_META=reviewManifest.concepts.filter(item=>item.disposition==="NO_SHEET_INTEGRATION_META").length;reviewManifest.generatedAt=GENERATED_AT;reviewManifest.composerLibraryVersion=library.libraryVersion;reviewSource.generatedAt=GENERATED_AT;reviewSource.composerLibraryVersion=library.libraryVersion;
