...
await test('Expected quota 429 exposes Retry-After through external CORS',async h=>{const c=await h.seed();assert.equal((await h.post([event()],c,{headers:{origin:external}})).status,202);const r=await h.post([event()],c,{headers:{origin:external}});assert.equal(r.status,429);readable(r);assert(Number(r.headers.get('retry-after'))>=1&&Number(r.headers.get('retry-after'))<=60);assert.equal(r.headers.get('access-control-expose-headers'),'Retry-After');},{MQ_CAP_CAP_EVENTS_MINUTE:'1'});
