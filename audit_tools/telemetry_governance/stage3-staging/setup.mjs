...
if(!db.uuid)throw Error('New D1 UUID missing');save({name,databaseName:name,databaseId:db.uuid,account,createdAt:new Date().toISOString(),syntheticOnly:true,workerDeployed:false,status:'AWAITING_STAGING_DEPLOYMENT',migrations:[],cleanupDecision:'Delete this new Worker and this new D1 after evidence capture'});
const migrations=path.join(root,'server/anonymous-telemetry-poc/migrations');
for(const file of ['0001_initial.sql','0002_completion_semantics.sql','0003_build_ingest_capabilities.sql']){
