...
export const out=path.join(root,'validation_artifacts/portable_telemetry_stage3_staging');
export const name='masteryquests-telemetry-stage3-staging';
...
export async function api(suffix,method='GET',body){
  ...
  const allowed=new Set(['/d1/database','/workers/subdomain',`/workers/scripts/${name}`,`/workers/scripts/${name}/subdomain`,`/workers/scripts/${name}/settings`,`/workers/scripts/${name}/tails`]);
