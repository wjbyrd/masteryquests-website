...
(async()=>{
 const recipe={schemaVersion:'1.2.0',title:'Central Banking Mastery Quest',slug:'central-banking-mastery-quest',supportedModes:['standard','timed','exam','legendary','score'],selectedConceptIds:['central-bank-and-federal-reserve','money-functions-and-measures','bank-money-creation','monetary-policy-tools'],checkpointFocus:{checkpointOne:null,checkpointTwo:null,finalCheckpoint:null}};
 ...
 fs.writeFileSync(path.join(root,'tests','stage1-central-banking-sample.html'),html);
 fs.writeFileSync(path.join(root,'tests','stage1-central-banking-sample-manifest.json'),JSON.stringify({...metadata,poolCounts:composition.counts},null,2));
