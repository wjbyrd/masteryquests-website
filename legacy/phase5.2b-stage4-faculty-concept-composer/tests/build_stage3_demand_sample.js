...
(async()=>{
 const recipe={schemaVersion:'1.2.0',title:'Demand Mastery Quest',slug:'demand-mastery-quest',supportedModes:['standard','timed','exam','legendary','score'],selectedConceptIds:['demand'],checkpointFocus:{checkpointOne:null,checkpointTwo:null,finalCheckpoint:null}};
 ...
 fs.writeFileSync(path.join(root,'tests','stage3-demand-sample.html'),html);
 fs.writeFileSync(path.join(root,'tests','stage3-demand-sample-manifest.json'),JSON.stringify({...metadata,poolCounts:composition.counts},null,2));
