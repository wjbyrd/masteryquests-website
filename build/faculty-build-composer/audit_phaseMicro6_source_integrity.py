...
if len(bp)!=7761:issues.append(f'baseline physical {len(bp)}')
if len(fp)!=7855:issues.append(f'final physical {len(fp)}')
...
new=set(fp)-set(bp)
if len(new)!=94:issues.append(f'new IDs {len(new)} != 94')
# Exactly original 430 Monopoly records may receive taxonomy metadata.
pc_base=set();m=b['concepts']['monopoly']
...
if set(tax_changed)!=pc_base:issues.append(f'taxonomy metadata changed on {len(tax_changed)} protected records, expected exact Monopoly 430')
...
result={'phase':'phaseMicro6-monopoly-source-integrity-v1','ok':not issues,'baselineCanonicalPhysical':len(bp),'finalCanonicalPhysical':len(fp),'protectedSemanticChanges':len(changed),'protectedTaxonomyMetadataChanges':len(tax_changed),'newQuestionIds':len(new),'baselineAssets':len(bh),'finalAssets':len(fh),'assetsByteIdentical':bh==fh,'issues':issues}
(FINAL/'phaseMicro6_monopoly_source_integrity_results.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2));raise SystemExit(0 if result['ok'] else 1)
