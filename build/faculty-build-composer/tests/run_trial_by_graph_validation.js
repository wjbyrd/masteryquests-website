...
(async()=>{
  ...
  if(library.canonicalQuestionCount!==8771) issues.push(`canonical count ${library.canonicalQuestionCount}`);
    ...
    for(const items of Object.values(module.questions||{})){
      ...
      for(const q of items){
        if(q.graphRequired===true){
          ...
          if(!auditIds.has(id) && !phase3eIds.has(id) && !externalitiesIds.has(id) && !publicGoodsIds.has(id) && !factorMarketIds.has(id)) flaggedOutsideAudit++;
  ...
  if(flagged!==861) issues.push(`graphRequired count ${flagged}`);
