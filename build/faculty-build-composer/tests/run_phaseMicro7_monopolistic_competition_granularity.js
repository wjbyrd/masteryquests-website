...
const qfile=JSON.parse(fs.readFileSync(path.join(root,'phaseMicro7-monopolistic-competition-granularity-adaptive-backfill-v1_questions.json'),'utf8')).questions;
const quality=JSON.parse(fs.readFileSync(path.join(root,'phaseMicro7_monopolistic_competition_quality_audit_results.json'),'utf8'));
const parentId='monopolistic-competition';
const children=['mcomp-structure-differentiation','mcomp-short-run-choice','mcomp-entry-exit-long-run','mcomp-advertising-nonprice','mcomp-efficiency-variety-limits'];
const supporting=new Set(['mcomp-structure-differentiation','mcomp-efficiency-variety-limits']);
...
const expectedParents={elasticity:495,'consumer-and-producer-surplus':442,'international-trade-and-trade-policy':487,'costs-of-production':608,'perfect-competition':565,monopoly:524,'monopolistic-competition':459};
function recipe(ids,modes=['standard','timed','exam','legendary','score']){return {schemaVersion:'1.2.0',title:'M7 Monopolistic Competition Validation',slug:'m7-monopolistic-competition-validation',supportedModes:modes,selectedConceptIds:ids,checkpointFocus:{checkpointOne:null,checkpointTwo:null,finalCheckpoint:null}}}
...
(async()=>{
 const issues=[],detail={};
 if(core.COMPOSER_VERSION!=='4.5n.0')issues.push(`core version ${core.COMPOSER_VERSION}`);
 if(library.composerVersion!=='4.5n.0')issues.push(`library version ${library.composerVersion}`);
 if(library.canonicalQuestionCount!==7902)issues.push(`canonical count ${library.canonicalQuestionCount}`);
 if(library.conceptCount!==120)issues.push(`concept count ${library.conceptCount}`);
 ...
 if(parentIds.size!==459)issues.push(`Monopolistic Competition parent ${parentIds.size} != 459`);
 for(const q of records(parent)){const subs=q.subtopicIds||[];if(subs.length!==1)issues.push(`${core.idOf(q)} MCMP assignments=${subs.length}`);else if(!children.includes(subs[0]))issues.push(`${core.idOf(q)} unknown MCMP child ${subs[0]}`);}
 for(const id of children){
  const m=core.resolveConceptModule(library,id);if(!m){issues.push(`${id} resolve failed`);continue;}
  const rs=records(m),ids=idsOf(rs);for(const x of ids){if(union.has(x))issues.push(`MCMP overlap ${x}`);union.add(x);}
 ...
 if(union.size!==parentIds.size||[...parentIds].some(x=>!union.has(x)))issues.push(`MCMP recombination parent=${parentIds.size} children=${union.size}`);
 if(routed.size!==12)issues.push(`new repair/bridge routed ${routed.size}/12`);
 ...
 const graphSource='phase6.2h-monopolistic-competition-graph-expansion-v2';
 const graphQs=Object.values(parent.questions||{}).flat().filter(q=>(q.sourceOccurrences||[]).some(o=>o.sourceFile===graphSource));
 const graphCounts={};for(const q of graphQs){const m=(q.image||'').match(/MCOMP-(\d\d)\.webp$/);if(m)graphCounts[m[1]]=(graphCounts[m[1]]||0)+1;}
 const expectedGraphs={'01':14,'02':12,'03':16};for(const [g,n] of Object.entries(expectedGraphs))if(graphCounts[g]!==n)issues.push(`MCMP graph ${g} ${graphCounts[g]||0}/${n}`);if(graphQs.length!==42)issues.push(`MCMP graph source count ${graphQs.length}/42`);
 const totalGraph=records(parent).filter(q=>q.image).length;if(totalGraph!==55)issues.push(`MCMP total graph coverage ${totalGraph}/55`);
 const mcAssets=library.assetInventory.filter(a=>a.conceptId===parentId);if(mcAssets.length!==38)issues.push(`MCMP assets ${mcAssets.length}/38`);for(const a of mcAssets){const p=path.join(root,'data',a.runtimePath);if(!fs.existsSync(p)){issues.push(`missing asset ${a.runtimePath}`);continue;}const h=crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex');if(h!==a.sha256)issues.push(`asset hash ${a.runtimePath}`);}
 const familyChildren={
  ...
  monopoly:['monopoly-power-barriers','monopoly-demand-revenue','monopoly-output-price','monopoly-profit-loss-shutdown','monopoly-welfare-efficiency','natural-monopoly-regulation','monopoly-price-discrimination'],
  'monopolistic-competition':children
 };
 ...
 const result={phase:'phaseMicro7-monopolistic-competition-granularity-adaptive-backfill-v1',ok:issues.length===0,composerVersion:core.COMPOSER_VERSION,canonicalQuestionCount:library.canonicalQuestionCount,conceptCount:library.conceptCount,parentCanonical:parentIds.size,childrenUnion:union.size,newAdaptiveSupportRouted:`${routed.size}/12`,qualityAuditOk:quality.ok,graphExpansion:{sourceQuestions:graphQs.length,graphCounts,totalGraphCoverage:totalGraph,assets:mcAssets.length},children:detail,regression,legacyRecipes:{passed:pass,total},issues};
