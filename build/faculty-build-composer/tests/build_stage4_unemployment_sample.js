...
(async()=>{
 const recipe={schemaVersion:'1.2.0',title:'Unemployment Measurement Mastery Quest',slug:'unemployment-measurement-mastery-quest',supportedModes:['standard','timed','exam','legendary','score'],selectedConceptIds:['unemployment-measurement'],checkpointFocus:{checkpointOne:null,checkpointTwo:null,finalCheckpoint:null}};
 ...
 fs.writeFileSync(path.join(root,'tests','stage4-unemployment-sample.html'),html);
 fs.writeFileSync(path.join(root,'tests','stage4-unemployment-sample-manifest.json'),JSON.stringify({...metadata,poolCounts:composition.counts},null,2));
