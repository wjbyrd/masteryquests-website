...
async function buildCase(name,supportedModes){
  const recipe={schemaVersion:'1.2.0',title:`Mode Test ${name}`,slug:`mode-test-${name}`,supportedModes,selectedConceptIds:['oligopoly'],checkpointFocus:{checkpointOne:null,checkpointTwo:null,finalCheckpoint:null}};
...
(async()=>{
  ...
  const cases=[
    ...
    await buildCase('all-five',['standard','timed','exam','legendary','score'])
  ];
  const issues=[];
  if(core.COMPOSER_VERSION!=='4.5p.0') issues.push(`composer version ${core.COMPOSER_VERSION}`);
  ...
  for(const c of cases){
    ...
    if(c.hidden.length!==5-c.requested.length) issues.push(`${c.name}: hidden count ${c.hidden.length}`);
    if(c.validation.some(v=>!v.ok)) issues.push(`${c.name}: selected mode preflight failed`);
  }
  const result={phase:'mode-availability-fix-v1',ok:issues.length===0,composerVersion:core.COMPOSER_VERSION,canonicalQuestionCount:library.canonicalQuestionCount,cssGuard,showReapply,returnReapply,cases,issues};
