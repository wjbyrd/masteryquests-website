...
function simulate(config){
  const modes=['standard','timed','exam','quiz','unlimited','legendary','score','trialGraph'];
...
(async()=>{
  ...
  for(const c of cases){
    ...
    if(c.hidden.length!==8-c.requested.length) issues.push(`${c.name}: hidden count ${c.hidden.length}`);
    if(c.validation.some(v=>!v.ok)) issues.push(`${c.name}: selected mode preflight failed`);
  }
  const result={phase:'mode-availability-fix-v4-eight-mode-aware',ok:issues.length===0,composerVersion:core.COMPOSER_VERSION,canonicalQuestionCount:library.canonicalQuestionCount,cssGuard,showReapply,returnReapply,cases,issues};
