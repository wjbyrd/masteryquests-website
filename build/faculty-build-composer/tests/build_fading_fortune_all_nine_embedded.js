...
(async()=>{
 const recipe={schemaVersion:core.RECIPE_SCHEMA_VERSION,title:'Fading Fortune Validation Game',slug:'fading-fortune-validation-game',supportedModes:[...core.MODE_ORDER],selectedConceptIds:['perfect-competition'],checkpointFocus:{checkpointOne:null,checkpointTwo:null,finalCheckpoint:null}};
 ...
 const html=core.buildHtml(template,c,config,metadata); fs.writeFileSync(path.join(root,'tests','fading-fortune-all-nine-embedded.html'),html);
 const ffIds=c.fadingFortuneQuestionIds||[]; const unique=new Set(ffIds);
 const out={ok:true,composerVersion:core.COMPOSER_VERSION,modes:core.MODE_ORDER,fadingFortuneEligible:c.counts.fadingFortuneEligible,fadingFortuneByDifficulty:c.counts.fadingFortuneByDifficulty,fadingFortuneQuestionIds:ffIds.length,uniqueFadingFortuneIds:unique.size,graphSafe:c.counts.graphSafe,assets:c.assets.length,htmlBytes:Buffer.byteLength(html),configFadingFortune:config.fadingFortune,validation:c.validation.modes.map(m=>({mode:m.mode,ok:m.ok,deficiencies:m.deficiencies.length,issues:m.issues.length}))};
 if(ffIds.length!==unique.size) throw new Error('Duplicate Fading Fortune IDs');
 if(c.counts.fadingFortuneEligible < 20) throw new Error('Expected at least 20 Fading Fortune questions');
 if(!c.validation.modes.every(m=>m.ok)) throw new Error('A mode failed validation');
 fs.writeFileSync(path.join(root,'fading_fortune_build_validation_results_4.5s.2l.json'),JSON.stringify(out,null,2)); console.log(JSON.stringify(out,null,2));
})();
