...
async function run() {
  ...
  pass(core.COMPOSER_VERSION === "4.5s.3e", `Composer version ${core.COMPOSER_VERSION}`);
  pass(core.RECIPE_SCHEMA_VERSION === "1.4.0", `Recipe schema ${core.RECIPE_SCHEMA_VERSION}`);
  pass(library.composerVersion === "4.5s.3e", `Library Composer version ${library.composerVersion}`);
  pass(library.canonicalQuestionCount === 8211, `Canonical count ${library.canonicalQuestionCount}`);
  pass(manifest.canonicalQuestionCount === 8211, `Manifest canonical count ${manifest.canonicalQuestionCount}`);
