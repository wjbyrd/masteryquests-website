...
def physical(lib):
 ...
 return out
bp,fp=physical(b),physical(f); issues=[]
if len(bp)!=7484: issues.append(f'baseline physical {len(bp)}')
...
changed=[]
for qid in bp:
 if qid in fp and scrub(bp[qid])!=scrub(fp[qid]): changed.append(qid)
if changed: issues.append(f'protected semantic changes {len(changed)}')
new=set(fp)-set(bp)
if len(new)!=166: issues.append(f'new IDs {len(new)} != 166')
# Exactly the original 442 Costs records may receive taxonomy metadata.
cost_base=set()
m=b['concepts']['costs-of-production']
for arr in m.get('questions',{}).values(): cost_base.update(q.get('canonicalId') or q.get('id') for q in arr or [])
for k in ('repairQuestions','bridgeQuestions','repairSeedQuestions'): cost_base.update(q.get('canonicalId') or q.get('id') for q in m.get(k,[]) or [])
tax_changed=[]
for qid in bp:
 before=bp[qid];after=fp[qid]
...
if set(tax_changed)!=cost_base: issues.append(f'taxonomy metadata changed on {len(tax_changed)} protected records, expected exact Costs 442')
...
# Physical asset bytes under data/question-assets exact.
def filehash(root):
 out={}
 d=root/'data/question-assets'
 for p in d.rglob('*'):
...
result={'phase':'phaseMicro4-costs-source-integrity-v1','ok':not issues,'baselineCanonicalPhysical':len(bp),'finalCanonicalPhysical':len(fp),'protectedSemanticChanges':len(changed),'protectedTaxonomyMetadataChanges':len(tax_changed),'newQuestionIds':len(new),'baselineAssets':len(bh),'finalAssets':len(fh),'assetsByteIdentical':bh==fh,'issues':issues}
(FINAL/'phaseMicro4_costs_source_integrity_results.json').write_text(json.dumps(result,indent=2)+'\n')
