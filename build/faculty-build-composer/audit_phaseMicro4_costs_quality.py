...
QF=ROOT/'phaseMicro4-costs-granularity-adaptive-backfill-v1_questions.json'
...
issues=[]
# schema/hash/reference checks
ids=set(); stems=set(); concept_ids=set(lib['concepts'])
correct_lengths=[]; all_lengths=[]; pos=Counter(); skill=Counter(); sub=Counter(); pool=Counter()
for q in new:
    qid=q['id'];
    ...
    if len(matches)!=1: issues.append(f'answer hash {qid} matches {len(matches)}')
        ...
        pos[matches[0]]+=1; correct_lengths.append(len(q['options'][matches[0]])); all_lengths.extend(len(o) for o in q['options'])
    for cid in q.get('secondaryConceptIds',[]):
        if cid not in concept_ids: issues.append(f'{qid} unknown secondary concept {cid}')
    if q.get('familyConceptId')!='costs-of-production' or len(q.get('subtopicIds',[]))!=1: issues.append(f'{qid} family/subtopic metadata')
    skill[q.get('primarySkill')]+=1; sub[q['subtopicIds'][0]]+=1; pool[q['sourcePool']]+=1
    if q.get('sourcePool') in ('repair','bridge') and q.get('instructionalRole')!=q.get('sourcePool'): issues.append(f'{qid} support role mismatch')
    if q.get('sourcePool') in ('easy','medium','hard') and q.get('instructionalRole')!='main': issues.append(f'{qid} main role mismatch')
# option-length diagnostic; flag only egregious systematic bias
avg_correct=sum(correct_lengths)/len(correct_lengths); avg_all=sum(all_lengths)/len(all_lengths)
longest_correct=0
for q in new:
    matches=[i for i,o in enumerate(q['options']) if answer_hash(o)==q.get('aHash')]
    if matches:
...
# near duplicate audit against new set and protected library, question stem only
...
newid=set(q['id'] for q in new)
protected=[q for q in all_existing if (q.get('id') or q.get('canonicalId')) not in newid]
flags=[]
new_norm=[norm(q['q']) for q in new]
prot_norm=[norm(q.get('q','')) for q in protected]
# exact/very-high similarity candidates only; then require token overlap so common repair phrasing does not create noise
