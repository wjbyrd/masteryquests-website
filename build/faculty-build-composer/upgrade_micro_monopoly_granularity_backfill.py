...
QFILE=ROOT/'phaseMicro6-monopoly-granularity-adaptive-backfill-v1_questions.json'
PHASE='phaseMicro6-monopoly-granularity-adaptive-backfill-v1'; GEN='2026-08-11T02:35:00.000Z'; COMPOSER_VERSION='4.5m.0'
PARENT_ID='monopoly'; PARENT_TITLE='Monopoly'
...
SUBTOPICS={
'monopoly-power-barriers':{'title':'Monopoly Power, Barriers to Entry & Sources of Monopoly','description':'Identify durable monopoly power and distinguish legal barriers, resource control, network effects, and natural-monopoly cost conditions.','role':'supporting-subtopic','objectives':['MON.1'],'related':['monopoly-demand-revenue','natural-monopoly-regulation']},
'monopoly-demand-revenue':{'title':'Monopoly Demand, Revenue & Marginal Revenue','description':'Interpret the monopolist’s downward-sloping demand, average revenue, marginal revenue, price effect, output effect, and elasticity-MR relationship.','role':'standalone-ready','objectives':['MON.2'],'related':['price-elasticity-of-demand','monopoly-output-price']},
'monopoly-output-price':{'title':'Profit-Maximizing Output & Monopoly Price','description':'Apply MR = MC with the rising-MC condition to choose monopoly output, then use demand to determine the corresponding price.','role':'standalone-ready','objectives':['MON.3'],'related':['marginal-analysis','monopoly-demand-revenue','monopoly-profit-loss-shutdown']},
'monopoly-profit-loss-shutdown':{'title':'Monopoly Profit, Loss & Shutdown','description':'Calculate monopoly profit or loss, interpret break-even, and distinguish producing at a loss from short-run shutdown.','role':'standalone-ready','objectives':['MON.4'],'related':['profit-concepts','sunk-avoidable-costs','monopoly-output-price']},
'monopoly-welfare-efficiency':{'title':'Monopoly Welfare, Efficiency & Competitive Comparison','description':'Compare monopoly with competition using output, price, consumer surplus, deadweight loss, allocative inefficiency, and rent seeking.','role':'standalone-ready','objectives':['MON.5'],'related':['consumer-and-producer-surplus','perfect-competition','monopoly-output-price']},
'natural-monopoly-regulation':{'title':'Natural Monopoly & Regulation','description':'Analyze natural-monopoly cost structure and evaluate marginal-cost pricing, average-cost pricing, price caps, subsidies, and regulatory tradeoffs.','role':'standalone-ready','objectives':['MON.6'],'related':['long-run-average-cost-scale','monopoly-welfare-efficiency','monopoly-power-barriers']},
'monopoly-price-discrimination':{'title':'Price Discrimination','description':'Analyze the conditions for discrimination, resale prevention, third-degree pricing, first-degree discrimination, elasticity, and welfare effects.','role':'standalone-ready','objectives':['MON.6'],'related':['price-elasticity-of-demand','monopoly-demand-revenue','monopoly-welfare-efficiency']},
}
REG_SKILLS={'marginal_cost_regulation','average_cost_regulation','regulatory_tradeoff','price_cap_regulation','natural_monopoly_cost','monopoly_policy_evaluation'}
PD_SKILLS={'third_degree_price_discrimination','first_degree_price_discrimination','price_discrimination_welfare','price_discrimination_conditions','resale_prevention'}
...
def classify(q):
    ...
    if o=='MON.6':
        if sk in REG_SKILLS: return 'natural-monopoly-regulation'
        if sk in PD_SKILLS: return 'monopoly-price-discrimination'
...
assert len(old_records)==430,len(old_records)
...
pe['coverageStatusNote']=pe.get('coverageStatusNote','').rstrip()+ ' Monopoly now exposes seven granular Micro child selectors and adds only the measured Easy/Medium/Hard, Repair, and Bridge backfill required for healthy adaptive use.'
...
for sid,spec in SUBTOPICS.items():
    ...
    child_entries.append({
        ...
        'excludedNeighboringSkills':['Other Monopoly material remains in the parent family or sibling subtopics.'],
        'prerequisiteConceptIds':['costs-of-production','price-elasticity-of-demand','marginal-analysis'],'relatedConceptIds':spec['related'],'sourceChapters':pe.get('sourceChapters',[]),'sourceObjectives':spec['objectives'],'sourceGames':source_games or pe.get('sourceGames',[]),
        ...
        'notes':'Derived Micro granularity slice of Monopoly with targeted adaptive-depth/support backfill; protected pre-Phase-Micro6 question wording and answers remain unchanged.',
        'instructionalClassification':'Monopoly subtopic','coverageStatus':'supporting-subtopic' if supporting else 'ready-focused-subtopic','coverageStatusLabel':'Best paired with another Monopoly topic' if supporting else 'Ready for focused use',
...
lib['registry']['generatedAt']=GEN; lib['registry']['curationPhase']=PHASE; lib['registry']['curationSummary']='Monopoly granularity and adaptive maturation: expose seven child selectors and add exactly 94 measured Easy/Medium/Hard, Repair, and Bridge questions. No Legendary, checkpoint, calculation, graph, or protected-question rewriting.'
...
# Keep runtime/core version in sync.
core=ROOT/'composer-core.js'; txt=core.read_text(encoding='utf-8'); txt=txt.replace("const COMPOSER_VERSION = '4.5l.0';", "const COMPOSER_VERSION = '4.5m.0';"); core.write_text(txt,encoding='utf-8')
...
(ROOT/'phaseMicro6_monopoly_granularity_metadata_results.json').write_text(json.dumps(summary,indent=2,ensure_ascii=False)+'\n',encoding='utf-8')
