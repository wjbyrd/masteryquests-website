...
function cloneQuestion(question, conceptId, assetConceptId = conceptId){
  ...
  if(copy.image){
    const normalized = String(copy.image).replaceAll('\\', '/').replace(/^data\//, '');
    const filename = normalized.split('/').pop();
    copy.image = normalized.includes('/')
      ? normalized
      : `question-assets/${assetConceptId || conceptId}/${filename}`;
  }
  return copy;
}
...
function validateGeneratedGraphAssets(composition){
  const normalizePath = value => String(value || '').replaceAll('\\', '/').replace(/^data\//, '');
  const assets = composition.assets || [];
  const embedded = composition.embeddedQuestionAssets || {};
  const embeddedByPath = new Map(Object.entries(embedded).map(([key, value]) => [normalizePath(key), value]));
  ...
  ]).filter(question => question?.graphRequired === true);
  const issues = [];
  for(const question of questions){
    const id = idOf(question) || 'unknown';
    const image = normalizePath(question.image || question.asset);
    const asset = assets.find(candidate => [candidate?.runtimePath, candidate?.sourceAssetPath, candidate?.sourceUrl]
      .some(candidatePath => normalizePath(candidatePath) === image));
    if(!image){
      issues.push({id, issue:'graphRequired question has no image reference'});
      continue;
    }
    if(!asset){
      issues.push({id, image, issue:'generated graph reference has no exact registered asset'});
      continue;
    }
    if(!asset.runtimePath || !asset.sourceUrl || !asset.sha256 || !Number.isFinite(asset.sizeBytes)){
      issues.push({id, image, issue:'registered graph asset lacks runtime, source, hash, or size metadata'});
      continue;
    }
    const dataUrl = embeddedByPath.get(normalizePath(asset.runtimePath));
    if(typeof dataUrl !== 'string' || !/^data:image\/[a-z0-9.+-]+;base64,/i.test(dataUrl)){
      issues.push({id, image, issue:'registered graph asset was not embedded into generated Quest output'});
    }
  }
  return {ok:issues.length === 0, questionCount:questions.length, assetCount:assets.length, issues};
}
