...
QF=ROOT/'phaseMicro5-perfect-competition-granularity-adaptive-backfill-v1_questions.json'
...
issues=[]; ids=set(); stems=set(); correct_lengths=[]; all_lengths=[]; pos=Counter(); skill=Counter(); sub=Counter(); pool=Counter(); correct_texts=[]
for q in new:
    qid=q['id'];
    ...
    if len(matches)!=1: issues.append(f'answer hash {qid} matches {len(matches)}')
        ...
        pos[matches[0]]+=1; ans=q['options'][matches[0]]; correct_lengths.append(len(ans)); correct_texts.append(ans); all_lengths.extend(len(o) for o in q['options'])
    if q.get('familyConceptId')!='perfect-competition' or len(q.get('subtopicIds',[]))!=1: issues.append(f'{qid} family/subtopic metadata')
...
# Protected records are everything except the 111 new IDs in physical modules.
all_existing=[]
