...
if len(bp)!=7902:issues.append(f'baseline physical {len(bp)}')
if len(fp)!=7977:issues.append(f'final physical {len(fp)}')
...
new=set(fp)-set(bp)
if len(new)!=75:issues.append(f'new IDs {len(new)} != 75')
base_family=set();m=b['concepts']['oligopoly']
...
if set(tax_changed)!=base_family:issues.append(f'taxonomy metadata changed on {len(tax_changed)} protected records, expected exact Oligopoly 370')
...
result={'phase':'phaseMicro8-oligopoly-source-integrity-v1','ok':not issues,'baselineCanonicalPhysical':len(bp),'finalCanonicalPhysical':len(fp),'protectedSemanticChanges':len(changed),'protectedTaxonomyMetadataChanges':len(tax_changed),'newQuestionIds':len(new),'baselineAssets':len(bh),'finalAssets':len(fh),'assetsByteIdentical':bh==fh,'issues':issues}
(FINAL/'phaseMicro8_oligopoly_source_integrity_results.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2));raise SystemExit(0 if result['ok'] else 1)
