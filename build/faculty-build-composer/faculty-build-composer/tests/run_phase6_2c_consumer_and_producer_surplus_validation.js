...
const conceptId = 'consumer-and-producer-surplus';
(async()=>{
  const recipe = {
    schemaVersion:'1.2.0',
    title:'Consumer and Producer Surplus Mastery Quest',
    slug:'consumer-and-producer-surplus-mastery-quest',
  ...
  const templateSha=await core.sha256Hex(template);
  const config=await core.createConfig(recipe,library,templateSha);
  ...
  const htmlPath=path.join(root,'tests','consumer-and-producer-surplus-sample.html');
  fs.writeFileSync(htmlPath,html);
  const expected={easy:30,medium:30,hard:60,elite:30,legendary:90,easyBoss:18,mediumBoss:18,finalBoss:18,legendaryBoss:36,repair:20,bridge:20,calculation:30,graph:64,assets:12,totalCanonical:370};
  const countIssues=[];
  for(const [key,value] of Object.entries(expected)) if(composition.counts[key]!==value) countIssues.push({key,expected:value,actual:composition.counts[key]});
  const result={
    ok:composition.errors.length===0 && answerAudit.ok && assetIssues.length===0 && countIssues.length===0 && composition.validation.modes.every(m=>m.ok),
    conceptId,
    ...
    countIssues,
    modes:composition.validation.modes,
    answerAudit,
    assetAudit:{assetCount:composition.assets.length,issues:assetIssues},
    sampleHtml:{path:'tests/consumer-and-producer-surplus-sample.html',bytes:Buffer.byteLength(html)},
    bossCoverage:composition.bossCoverage
  };
  fs.writeFileSync(path.join(root,'tests','phase6_2c_consumer_and_producer_surplus_validation_results.json'),JSON.stringify(result,null,2));
