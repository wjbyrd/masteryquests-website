...
if len(bp)!=7855:issues.append(f'baseline physical {len(bp)}')
if len(fp)!=7902:issues.append(f'final physical {len(fp)}')
...
new=set(fp)-set(bp)
if len(new)!=47:issues.append(f'new IDs {len(new)} != 47')
base_family=set();m=b['concepts']['monopolistic-competition']
for arr in m.get('questions',{}).values():base_family.update(q.get('canonicalId') or q.get('id') for q in arr or [])
for k in ('repairQuestions','bridgeQuestions','repairSeedQuestions'):base_family.update(q.get('canonicalId') or q.get('id') for q in m.get(k,[]) or [])
tax_changed=[]
for qid in bp:
...
if set(tax_changed)!=base_family:issues.append(f'taxonomy metadata changed on {len(tax_changed)} protected records, expected exact Monopolistic Competition 412')
...
result={'phase':'phaseMicro7-monopolistic-competition-source-integrity-v1','ok':not issues,'baselineCanonicalPhysical':len(bp),'finalCanonicalPhysical':len(fp),'protectedSemanticChanges':len(changed),'protectedTaxonomyMetadataChanges':len(tax_changed),'newQuestionIds':len(new),'baselineAssets':len(bh),'finalAssets':len(fh),'assetsByteIdentical':bh==fh,'issues':issues}
(FINAL/'phaseMicro7_monopolistic_competition_source_integrity_results.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2));raise SystemExit(0 if result['ok'] else 1)
