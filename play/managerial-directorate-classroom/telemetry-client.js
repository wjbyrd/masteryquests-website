(function anonymousClassroomTelemetry(){
  ...
  const BUILD_VERSION = "2026.09.10-classroom-parity3";
  const SCHEMA_VERSION = 3;
  const DEFAULT_ENDPOINT = "/api/anonymous-telemetry-poc/v1/events";
  const MAX_QUEUE = 2000;
  let collectionEpoch=0, memoryRemoteDisabled=false;
  // Per-page bounded transport recovery; never persisted as capability revocation.
  let transientRetries=0, transientStartedAt=null, retryNotBefore=0;
  ...
  function activeDescriptor(){
    ...
    try{
      ...
      if(safeStorageGet(BUILD_SCOPE+':terminal-activation:'+descriptor.capabilityId)==='1')return null;
      return descriptor;
    }catch(_){return null;}
  }
  function suspendDescriptor(descriptor){
    memoryRemoteDisabled=true;collectionEpoch++;
    try{safeStorageSet(BUILD_SCOPE+':terminal-activation:'+descriptor.capabilityId,'1');}catch(_){}
  ...
  function saveQuality(){try{safeStorageSet(QUALITY_KEY,JSON.stringify(state.quality));}catch(_){state.quality.storageFailures++;}}
...
function createTelemetryContract(get, registry) {
  ...
  function state(run){
    ...
    let s;try{s=JSON.parse(safeStorageGet(key(run))||'null');}catch(_){}
  ...
  function writeJSON(key, value) {
    try { safeStorageSet(key, JSON.stringify(value)); } catch (_) { state.quality.storageFailures++; }
  }
  ...
  const state = {
    ...
    activeRunId: readActiveRunId(ACTIVE_RUN_KEY),
    sourceRunId: "",
    flushing: false,
    retryAttempt: 0,
    retryTimer: 0,
  ...
  function scheduleFlush(delay) {
    ...
    if(IS_COMPOSER)delay=Math.max(delay,retryNotBefore-Date.now());
    if (state.retryTimer) clearTimeout(state.retryTimer);
    state.retryTimer = setTimeout(() => flush(), delay);
  }

  function retryTransient(serverDelay=0) {
    const now=Date.now();
    if(transientStartedAt===null)transientStartedAt=now;
    const delay=Math.max(serverDelay,1000*2**transientRetries)+Math.floor(Math.random()*1001);
    if(transientRetries>=3 || now+delay-transientStartedAt>300000){
      // Stop this page's transport, without permanently invalidating a healthy build.
      memoryRemoteDisabled=true;collectionEpoch++;
      clearTimeout(state.retryTimer);state.retryTimer=0;
      state.quality.cancelled+=state.queue.length;state.queue=[];persistQueue();saveQuality();
      return {ok:false,retryExhausted:true};
    }
    transientRetries++;retryNotBefore=now+delay;scheduleFlush(delay);
    return {ok:false,retryable:true,retryAfterMs:delay,queued:state.queue.length};
  }

  async function flush(options = {}) {
    if(!remoteEnabled()){setRemoteCollection(false);return {ok:true,disabled:true,queued:0};}
    const epoch=collectionEpoch;
    if(IS_COMPOSER&&transientStartedAt!==null&&Date.now()-transientStartedAt>300000)return retryTransient();
    if(IS_COMPOSER&&Date.now()<retryNotBefore)return {ok:false,retryable:true,queued:state.queue.length};
    ...
    try {
      ...
      if(descriptor&&response.status===429){
        const rejection=await response.json().catch(()=>({}));
        if(epoch!==collectionEpoch||!remoteEnabled())return {ok:false,cancelled:true};
        if(rejection.error==='ingest_rate_limited'){
          const value=response.headers.get('Retry-After');
          const seconds=/^\d+$/.test(value||'')?Number(value):null;
          if(seconds!==null&&!Number.isFinite(seconds))return retryTransient(300001);
          const delay=seconds!==null?seconds*1000:Math.max(0,Date.parse(value)-Date.now());
          return retryTransient(Number.isFinite(delay)?Math.max(1000,delay):60000);
        }
        suspendDescriptor(descriptor);return {ok:false,terminalRejection:true};
      }
      if(descriptor&&[401,403].includes(response.status)){suspendDescriptor(descriptor);return {ok:false,terminalRejection:true};}
      ...
      transientRetries=0;transientStartedAt=null;retryNotBefore=0;
      ...
      if(IS_COMPOSER)return retryTransient();
  ...
  function addDisclosure() {
    ...
    disclosure.innerHTML = "<button type='button' class='game-menu-option' aria-expanded='false' aria-controls='anonymousTelemetryDetails'>About this class build</button><p id='anonymousTelemetryDetails' hidden>Remote anonymous collection is OFF by default and requires explicit instructor/deployer enablement. When enabled, this private or class-specific build sends anonymous gameplay and interaction telemetry to Mastery Quests for operational, instructional, and product analytics, not research by default. Separately designated research requires its own institutional/research decisions and dataset handling. Names, email addresses, LMS account identifiers, university identifiers, and free-text responses are not intentionally included in application telemetry. This build records game interactions, including answers, timing, and counts of relevant selection/copy actions. It never stores selected or copied content, clipboard contents, keystrokes, or unrelated-tab activity. A random browser ID can link runs without identifying a person. Use Download Game Data to inspect your own record. A tab switch or copy action alone does not establish misconduct, intent, attention, cognition, or learning. Turning off future transmission cancels pending unsent events and keeps local downloads available; the choice persists after refresh when browser storage is available. Re-enabling does not replay cancelled events. It cannot retract in-flight requests or delete accepted server records, downloaded copies, provider logs, or backups. Already accepted application data remains under our 730-day whole-run policy, automatically evaluated daily from the latest stored server receipt. Disclosure mq-disclosure/2; governance mq-governance/2. <a href='/privacy/'>Privacy and your choices</a></p>";
  ...
  try {
    const client=safeStorageGet(CLIENT_KEY);
