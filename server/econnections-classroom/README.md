...
- Creation makes an upcoming/open session available immediately. Before `studentWindowStart`, its effective status is upcoming; from that time until close it is open. The warmup window is not an access restriction. Do not circulate the token before access is intended.
- `walkthroughStart` is the analytical cutoff. A completion qualifies only when its Worker-generated receipt is **strictly earlier**. Equality is late. `elapsedMs` is active client time and never determines eligibility.
