...
const require=createRequire(import.meta.url),repo='C:/Users/Jennings/Documents/GitHub/masteryquests-website',work=path.resolve('.open-work');
...
for(const c of sources)for(const a of lib.concepts[c].assetMetadata||[]){
 ...
 if(!same(old,a)){
  assetChanges.push({concept:c,runtimePath:a.runtimePath,reason:read('accessibility-inspection-after.json').find(x=>x.runtimePath===a.runtimePath).reason,before:old,after:structuredClone(a)});
...
const recipe={schemaVersion:core.RECIPE_SCHEMA_VERSION,title:'Open-Economy Macro & Exchange Rates audit validation',slug:'open-economy-macro-audit',guideName:'Guide',selectedConceptIds:selected,supportedModes:core.MODE_ORDER,checkpointFocus:Object.fromEntries(core.CHECKPOINT_ORDER.map(k=>[k,null]))};
