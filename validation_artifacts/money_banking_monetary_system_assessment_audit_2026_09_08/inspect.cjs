...
const match=source.match(/id: 'money-banking-inflation',[\s\S]*?conceptIds: \[([\s\S]*?)\]/);
if(!match)throw Error('Preset missing');
const ids=[...match[1].matchAll(/'([^']+)'/g)].map(x=>x[1]);
const recipe={schemaVersion:core.RECIPE_SCHEMA_VERSION,title:'Money, Banking, Inflation & Monetary Policy',slug:'money-banking-inflation-audit',selectedConceptIds:ids,supportedModes:[...core.MODE_ORDER],checkpointFocus:Object.fromEntries(core.CHECKPOINT_ORDER.map(k=>[k,null]))};
