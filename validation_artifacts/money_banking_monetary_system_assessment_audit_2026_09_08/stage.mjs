...
const require=createRequire(import.meta.url),repo='C:/Users/Jennings/Documents/GitHub/masteryquests-website',work=path.resolve('.money-work');
...
const recipe={schemaVersion:core.RECIPE_SCHEMA_VERSION,title:'Money, Banking, Inflation & Monetary Policy audit validation',slug:'money-banking-inflation-audit',guideName:'Guide',selectedConceptIds:selected,supportedModes:core.MODE_ORDER,checkpointFocus:Object.fromEntries(core.CHECKPOINT_ORDER.map(k=>[k,null]))};
...
fs.mkdirSync(path.join(work,'generated'),{recursive:true});fs.writeFileSync(path.join(work,'generated/index.html'),core.buildHtml(template,comp,config,{generatedAt:'2026-09-08',purpose:'Local audit validation only'}));
