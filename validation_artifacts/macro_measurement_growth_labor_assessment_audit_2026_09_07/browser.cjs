...
(async()=>{
 const root=path.resolve('.macro-measure-work/generated');
 ...
 const changes=JSON.parse(fs.readFileSync('.macro-measure-work/changes.json','utf8'));
 ...
 await page.evaluate(()=>{currentQuestion=structuredClone(Object.values(questionBanks).filter(Array.isArray).flat().find(q=>q.id==='PM2B3-PROD-FB-004'));document.getElementById('question').textContent=currentQuestion.q;displayQuestion();});
 ...
 await page.screenshot({path:'.macro-measure-work/browser-smoke.png',fullPage:true});
 const report={engine:'Microsoft Edge / Playwright',visibleLegendaryQuestion:true,exampleId:'PM2B3-PROD-FB-004',pageErrors:errors,revisedRecordsVerified:verified.length,allRevisedRecordsPresent:true,graphChecks,externalRequestsBlocked:true,scope:'Generated HTML loads; all revised payloads match; all 12 graph questions decode and their click-to-enlarge image opens. This is a content smoke test, not an all-mode playthrough.'};
 if(errors.length)throw Error(JSON.stringify(errors));fs.writeFileSync('.macro-measure-work/browser-validation.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify({...report,graphChecks:graphChecks.length}));
