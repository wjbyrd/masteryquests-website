...
var ROOT = "/play/managerial-directorate-classroom";
var COOKIE = "__Secure-MQClassroom";
var LIFETIME = 10 * 60 * 60;
...
__name(pathForGate, "pathForGate");
function protectedPath(url) {
  const p = pathForGate(url);
  return p === ROOT || p.startsWith(ROOT + "/");
}
__name(protectedPath, "protectedPath");
function base64url(bytes) {
  return btoa(String.fromCharCode(...new Uint8Array(bytes))).replaceAll("+", "-").replaceAll("/", "_").replace(/=+$/, "");
}
__name(base64url, "base64url");
function unbase64url(value) {
  if (!/^[A-Za-z0-9_-]+$/.test(value)) throw Error("invalid");
...
async function sign(payload, key) {
  return base64url(await crypto.subtle.sign("HMAC", key, encoder.encode("classroom-session:" + payload)));
}
__name(sign, "sign");
...
var worker_default = {
  async fetch(request, env) {
    ...
    const key = await keyFor(env.CLASSROOM_SESSION_SECRET), now = Math.floor(Date.now() / 1e3);
    ...
    try {
      if (!env.CLASSROOM_LOGIN_LIMIT || !(await env.CLASSROOM_LOGIN_LIMIT.limit({ key: "managerial-classroom-login" })).success) return page("Please wait a minute before trying again.", 429);
    } catch {
      return page("", 503, true);
    }
    let submitted;
    try {
      ...
      if (fields.getAll("accessCode").length !== 1 || [...fields.keys()].some((k) => k !== "accessCode")) throw Error("invalid");
      submitted = fields.get("accessCode");
      if (submitted.length > 256) throw Error("large");
    } catch {
      return page("That access code is not valid.", 400);
    }
