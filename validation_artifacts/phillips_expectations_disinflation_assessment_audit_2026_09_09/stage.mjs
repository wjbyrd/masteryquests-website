...
const require=createRequire(import.meta.url),repo='C:/Users/Jennings/Documents/GitHub/masteryquests-website',work=path.resolve('.phillips-work');
...
const sources=read('sources.json'), scopedRecords=read('inventory-before.json').records, scopedIds=new Set(Object.keys(scopedRecords));
...
const descriptions=read('accessibility-spec.json');
...
for(const c of sources){
 ...
 for(const [p,q] of locations(m)){
  const id=String(q.id),s=spec[id],old=structuredClone(q); if(!scopedIds.has(id)){assert(!s,'Out-of-scope revision '+id);continue;}
  if(s){
  ...
  const access=descriptions[q.image]||descriptions[String(q.image||'').split('/').pop()];if(access){q.imageAlt=access.imageAlt;q.graphDescription=access.graphDescription;}
  ...
  if(!same(old,q)&&!recordChanges.has(id)){
   ...
   const change={id,concept:scopedRecords[id].primaryConceptId,authoritativeConcept:c,pool:p,difficulty:q.canonicalDifficulty||'unknown',objective:q.objective,type:q.type,primarySkill:q.primarySkill,contentRevised:!!s,pathRevised:old.image!==q.image,reason:s?.reason||(old.image!==q.image?'Repair legacy image reference and deliver its visually verified registered accessibility contract.':'Complete inline accessibility with the inspected graph evidence.'),numericProofs:s?.proof||[],before:Object.fromEntries(keys.map(k=>[k,old[k]])),after:Object.fromEntries(keys.map(k=>[k,q[k]]))};
...
for(const c of sources)for(const a of lib.concepts[c].assetMetadata||[]){
 ...
 const old=structuredClone(a);a.graphDescription=description.graphDescription;
 a.imageAlt=description.imageAlt;
 if(!same(old,a)){
  assetChanges.push({concept:c,runtimePath:a.runtimePath,reason:read('accessibility-inspection.json').find(x=>x.runtimePath===a.runtimePath).reason,before:old,after:structuredClone(a)});
...
const permitted=['q','options','aHash','feedback','image','imageAlt','graphDescription'];
...
for(const [c,m] of Object.entries(lib.concepts)){
 ...
 for(const [id,q] of now){
  const b=oldQ.get(id);if(!scopedIds.has(id)){assert(same(q,b),'Unselected parent record changed '+id);continue;}total++;
...
const recipe={schemaVersion:core.RECIPE_SCHEMA_VERSION,title:'Phillips Curve, Expectations & Disinflation audit validation',slug:'phillips-expectations-disinflation-audit',guideName:'Guide',selectedConceptIds:selected,supportedModes:core.MODE_ORDER,checkpointFocus:Object.fromEntries(core.CHECKPOINT_ORDER.map(k=>[k,null]))};
const comp=core.compose(lib,recipe),before=core.compose(base,recipe);
const fullRecipe={...recipe,supportedModes:[...core.MODE_ORDER]},fullReadiness=core.compose(lib,fullRecipe),oldFullReadiness=core.compose(base,fullRecipe);
assert(same(fullReadiness.validation,oldFullReadiness.validation)&&same(fullReadiness.errors,oldFullReadiness.errors),'All-mode baseline changed');
...
assert(same(parentBefore.counts,parentAfter.counts)&&same(parentBefore.validation,parentAfter.validation)&&same(parentBefore.errors,parentAfter.errors),'Parent preset readiness changed');
write('parent-preset-validation.json',{counts:parentAfter.counts,modes:parentAfter.validation,errors:parentAfter.errors,unchanged:true});
...
fs.mkdirSync(path.join(work,'generated'),{recursive:true});fs.writeFileSync(path.join(work,'generated/index.html'),core.buildHtml(template,comp,config,{generatedAt:'2026-09-09',purpose:'Local audit validation only'}));
const projection=structuredClone(lib);Object.assign(projection.concepts,views);fs.writeFileSync(path.join(work,'quality-projection-after.js'),'window.MQ_COMPOSER_LIBRARY='+JSON.stringify(projection)+';\n');
...
const summary={uniqueBefore:total,uniqueAfter:total,contentRevised:contentIds.size,pathRevised:pathIds.size,pathOnly:changes.filter(c=>!c.contentRevised&&c.pathRevised).length,assetMetadataOnly:changes.filter(c=>!c.contentRevised&&!c.pathRevised).length,recordUnchanged:total-changes.length,added:0,removed:0,contentByConcept:group(changes.filter(c=>c.contentRevised),'concept'),contentByDifficulty:group(changes.filter(c=>c.contentRevised),'difficulty'),contentByType:group(changes.filter(c=>c.contentRevised),'type'),contentByObjective:group(changes.filter(c=>c.contentRevised),'objective'),allChangesByDifficulty:group(changes,'difficulty'),accessibilityAssetRecords:assetChanges.length,graphCount:66,assetRuntimeCount:comp.assets.length,oldLibrarySha256:base.librarySha256,newLibrarySha256:lib.librarySha256,compositionCounts:comp.counts,modeValidation:comp.validation,fullPresetReadiness:fullReadiness.validation,fullPresetErrors:fullReadiness.errors,answerVerification:answers,graphValidation:graphs,warnings:comp.warnings,unrelatedAndDerivedDefinitionsUnchanged:Object.keys(lib.concepts).length-sources.length,conceptReviewRoutesUnchanged:true,allSourceRecordRoutingUnchanged:true,allPoolMembershipsAndOrderUnchanged:true,allResolvedViewMembershipsUnchanged:true,graphBytesUnchanged:true};
