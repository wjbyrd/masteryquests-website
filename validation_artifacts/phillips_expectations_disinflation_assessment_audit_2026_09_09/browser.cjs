...
(async()=>{
 const root=path.resolve('.phillips-work/generated');
 ...
 const changes=JSON.parse(fs.readFileSync('.phillips-work/changes.json','utf8'));
 ...
 await page.evaluate(()=>{currentQuestion=structuredClone(Object.values(questionBanks).filter(Array.isArray).flat().find(q=>String(q.id)==='PG5-PC-L-006'));document.getElementById('question').textContent=currentQuestion.q;displayQuestion();});
 ...
 await page.screenshot({path:'.phillips-work/browser-smoke.png',fullPage:true});
 const report={engine:'Microsoft Edge / Playwright',visibleLegendaryQuestion:true,exampleId:'PG5-PC-L-006',pageErrors:errors,revisedRecordsVerified:verified.length,allRevisedRecordsPresent:true,graphChecks,externalRequestsBlocked:true,scope:'Generated HTML loads; all revised payloads match; all ordinary/checkpoint graph questions decode and their click-to-enlarge image opens. This is a content smoke test, not an all-mode playthrough.'};
 if(errors.length)throw Error(JSON.stringify(errors));fs.writeFileSync('.phillips-work/browser-validation.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify({...report,graphChecks:graphChecks.length}));
