...
const require=createRequire(import.meta.url),repo='C:/Users/Jennings/Documents/GitHub/masteryquests-website',work=path.resolve('.firms-work');
...
for(const c of sources)for(const a of lib.concepts[c].assetMetadata||[]){
 ...
 if(!same(old,a)){
  assetChanges.push({concept:c,runtimePath:a.runtimePath,reason:'Replace garbled OCR, missing coordinates or inaccurate curve claims with visually verified graph observations.',before:old,after:structuredClone(a)});
...
const recipe={schemaVersion:core.RECIPE_SCHEMA_VERSION,title:'Firms and Markets audit validation',slug:'firms-markets-audit-validation',guideName:'Guide',selectedConceptIds:selected,supportedModes:core.MODE_ORDER,checkpointFocus:Object.fromEntries(core.CHECKPOINT_ORDER.map(k=>[k,null]))};
