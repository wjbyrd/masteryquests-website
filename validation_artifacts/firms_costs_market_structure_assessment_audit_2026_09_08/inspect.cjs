...
const match=source.match(/id: 'micro-firms-markets',[\s\S]*?conceptIds: \[([\s\S]*?)\]/);
if(!match)throw Error('Preset missing');
const ids=[...match[1].matchAll(/'([^']+)'/g)].map(x=>x[1]);
const recipe={schemaVersion:core.RECIPE_SCHEMA_VERSION,title:'Firms, Costs & Market Structure',slug:'micro-firms-markets-audit',selectedConceptIds:ids,supportedModes:[...core.MODE_ORDER],checkpointFocus:Object.fromEntries(core.CHECKPOINT_ORDER.map(k=>[k,null]))};
