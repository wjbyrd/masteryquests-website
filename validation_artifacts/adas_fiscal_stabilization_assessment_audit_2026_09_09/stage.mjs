...
const require=createRequire(import.meta.url),repo='C:/Users/Jennings/Documents/GitHub/masteryquests-website',work=path.resolve('.adas-work');
...
const protectedConcepts=read('overlap-mask.json').selectedPhillipsDeferredConcepts;
...
for(const c of sources){
 const m=lib.concepts[c]; if(protectedConcepts.includes(c))continue;
 for(const [p,q] of locations(m)){
  ...
  if(!same(old,q)&&!recordChanges.has(id)){
   ...
   const change={id,concept:scopedRecords[id].primaryConceptId,authoritativeConcept:c,pool:p,difficulty:q.canonicalDifficulty||'unknown',objective:q.objective,type:q.type,primarySkill:q.primarySkill,contentRevised:!!s,pathRevised:old.image!==q.image,reason:s?.reason||(old.image!==q.image?'Resolve the legacy image reference to its existing registered runtime asset path.':'Synchronize inline accessibility metadata with the visually verified approved asset contract.'),numericProofs:s?.proof||[],before:Object.fromEntries(keys.map(k=>[k,old[k]])),after:Object.fromEntries(keys.map(k=>[k,q[k]]))};
...
for(const c of sources.filter(c=>!protectedConcepts.includes(c)))for(const a of lib.concepts[c].assetMetadata||[]){
...
for(const c of protectedConcepts)assert(same(lib.concepts[c],base.concepts[c]),'Protected Phillips changed '+c);
...
const recipe={schemaVersion:core.RECIPE_SCHEMA_VERSION,title:'AD-AS, Fiscal Policy & Stabilization audit validation',slug:'adas-fiscal-stabilization-audit',guideName:'Guide',selectedConceptIds:selected,supportedModes:core.MODE_ORDER,checkpointFocus:Object.fromEntries(core.CHECKPOINT_ORDER.map(k=>[k,null]))};
