...
(async()=>{
 const root=path.resolve('.adas-work/generated');
 ...
 const changes=JSON.parse(fs.readFileSync('.adas-work/changes.json','utf8'));
 ...
 for(const id of graphIds){
  const check=await page.evaluate(async id=>{const q=Object.values(questionBanks).filter(Array.isArray).flat().find(q=>String(q.id)===id);renderQuestionMedia(q);document.getElementById('question').textContent=q.q;const imgs=[...document.querySelectorAll('#questionImageBox img')];await Promise.all(imgs.map(i=>i.decode().catch(()=>{})));imgs[0]?.click(); const zoom=document.getElementById('graphLightboxImg'); await zoom.decode().catch(()=>{}); const zoomOK=document.getElementById('graphLightbox').classList.contains('active')&&zoom.naturalWidth>0; closeGraphLightbox(); return {id,concept:q.primaryConceptId,zoomOK,description:document.getElementById('questionGraphDescription')?.textContent,source:imgs.map(i=>i.src.slice(0,180)),questionImage:q.image,embeddedImages:imgs.length,loaded:imgs.every(i=>i.naturalWidth>0),text:document.getElementById('question').textContent===q.q};},id);graphChecks.push(check);
 }
 const protectedConcepts=JSON.parse(fs.readFileSync('.adas-work/overlap-mask.json','utf8')).selectedPhillipsDeferredConcepts;
 if(graphChecks.some(c=>!protectedConcepts.includes(c.concept)&&(!c.loaded||!c.embeddedImages||!c.text||!c.zoomOK||!c.description)))throw Error('Graph browser render failed '+JSON.stringify(graphChecks));
 await page.evaluate(()=>{currentQuestion=structuredClone(Object.values(questionBanks).filter(Array.isArray).flat().find(q=>String(q.id)==='PG3-MEQ-L-004'));document.getElementById('question').textContent=currentQuestion.q;displayQuestion();});
 ...
 await page.screenshot({path:'.adas-work/browser-smoke.png',fullPage:true});
 const report={engine:'Microsoft Edge / Playwright',visibleLegendaryQuestion:true,exampleId:'PG3-MEQ-L-004',pageErrors:errors,revisedRecordsVerified:verified.length,allRevisedRecordsPresent:true,graphChecks,externalRequestsBlocked:true,scope:'Generated HTML loads; all revised payloads match; all core AD-AS/fiscal ordinary/checkpoint graph questions decode and their click-to-enlarge image opens; protected Phillips results are individually recorded. This is a content smoke test, not an all-mode playthrough.'};
 if(errors.length)throw Error(JSON.stringify(errors));fs.writeFileSync('.adas-work/browser-validation.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify({...report,graphChecks:graphChecks.length}));
