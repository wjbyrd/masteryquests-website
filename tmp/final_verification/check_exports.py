...
group = groups['general']; expected = e.make_rows(group, root, library)
with (root/'faculty_exports/general_economics_question_bank.csv').open(encoding='utf-8-sig', newline='') as f:
...
pdfpath = root/'faculty_exports/general_economics_question_bank.pdf'
...
(root/'tmp/final_verification/export_checks.json').write_text(json.dumps(result,indent=2),encoding='utf-8')
