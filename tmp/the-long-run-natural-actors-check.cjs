...
const beforeFile='tmp/the-long-run-pre-natural-actors.html',game='audit_tools/econ_rpg/game/games/the-long-run/index.html';
const old=fs.readFileSync(beforeFile,'utf8').replaceAll('\r\n','\n'),now=fs.readFileSync(game,'utf8').replaceAll('\r\n','\n');
const scrub=s=>s.replace(/walkFrameRate:4|walkStridePixels:8/,'WALK_CADENCE').replace(/\/\/ Native (16 × 26|18 × 27) people[\s\S]*?(?=const sprites=\{)/,'ACTOR_ART\n').replace(/locate\(object\);const phase=[^\n]+/,'ACTOR_PHASE');
...
for(const [name,p] of [['before',before],['after',page]]){
 const sheet=await p.evaluate(()=>{
  ...
  ['pedestrian','shopper','factoryWorker','worker','resident','compact','sedan','truck'].forEach((type,i)=>{ctx.fillStyle='#192e40';ctx.fillText(type,8,i*104+20);sprites[type].frames.forEach((frame,f)=>{frame.forEach((row,y)=>[...row].forEach((v,x)=>{if(v!==' '){ctx.fillStyle=PIXEL_PALETTE[v];ctx.fillRect(160+f*150+x*3,i*104+12+y*3,3,3)}}));});});return c.toDataURL().split(',')[1];
 });fs.writeFileSync(`tmp/long-run-actors-${name}.png`,Buffer.from(sheet,'base64'));
}
...
const check=await page.evaluate(()=>{
 const issues=new Set(),isCar=o=>['compact','sedan','truck'].includes(o.spriteType),key=o=>`${isCar(o)?'car':'human'}/${o.path}/${o.index}`;
 ...
 return {issues:[...issues],frames:pixelWorld.inspect().frameCount};
});assert.deepEqual(check.issues,[]);console.log('PASS: 192 seconds of simulated motion, lane clearances, sprite bounds/colors, furniture, wraparound and year continuity.',check);
for(const [label,ids] of Object.entries({baseline:[],busy:['confidence','fiscal-expand','rate-cut'],recession:['energy','fiscal-contract','rate-hike']})){
 ...
 await page.evaluate(()=>stepWorld(30));await page.locator('.world-window').screenshot({path:`tmp/long-run-natural-${label}.png`});
}
