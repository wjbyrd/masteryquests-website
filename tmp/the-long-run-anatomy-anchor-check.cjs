...
const beforeFile='tmp/the-long-run-pre-anatomy-anchor.html',game='audit_tools/econ_rpg/game/games/the-long-run/index.html';
...
for(const [name,p] of [['before',before],['after',page]]){
 ...
 });fs.writeFileSync(`tmp/long-run-anatomy-anchor-actors-${name}.png`,Buffer.from(sheet,'base64'));
}
const dimensions=await page.evaluate(()=>Object.fromEntries(Object.entries(sprites).filter(([type])=>SPRITE_PLACEMENT[type]).map(([type,s])=>[type,[s.frames[0][0].length,s.frames[0].length]])));assert.deepEqual(dimensions.compact,[44,22]);assert.deepEqual(dimensions.sedan,[50,22]);assert.deepEqual(dimensions.truck,[56,24]);assert.deepEqual(dimensions.pedestrian,[22,32]);
const check=await page.evaluate(()=>{
 ...
 function inspect(ticks){let prior=pixelWorld.inspect().objects;
  for(let tick=0;tick<ticks;tick++){stepWorld(1);const current=pixelWorld.inspect().objects;
   for(const lane of ['upperRoad','lowerRoad']){const cars=current.filter(o=>isCar(o)&&o.active&&o.path===lane).sort((a,b)=>a.x-b.x);for(let i=1;i<cars.length;i++)if(cars[i].x-cars[i-1].x<sprites[cars[i-1].spriteType].frames[0][0].length+6)issues.add('Car crowding');for(const car of cars){if(car.y!==(lane==='upperRoad'?197:218))issues.add('Lane drift');const y=vehicleDrawY(car.spriteType,car.path),contact=y+VEHICLE_WHEEL_CONTACT[car.spriteType];if(contact!==VEHICLE_ROAD_BASELINES[car.path]||contact-6<196||contact>=238)issues.add('Wheel contact off road');if(lane==='upperRoad'&&y>=196)issues.add('Roof trapped in road band');}}
...
for(const [label,ids] of Object.entries({baseline:[],busy:['confidence','fiscal-expand','rate-cut'],recession:['energy','fiscal-contract','rate-hike']})){
 ...
 await page.evaluate(()=>stepWorld(30));await page.locator('.world-window').screenshot({path:`tmp/long-run-anatomy-anchor-${label}.png`});
}
