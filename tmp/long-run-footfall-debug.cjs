...
const check=await page.evaluate(()=>{
 ...
 function inspect(ticks){let prior=pixelWorld.inspect().objects;
  for(let tick=0;tick<ticks;tick++){stepWorld(1);const current=pixelWorld.inspect().objects;
   ...
   for(const o of current){const previous=prior.find(p=>key(p)===key(o));if(o.path&&o.active&&previous?.active&&Math.abs(o.x-previous.x)>2)issues.add('Motion jump');if(!o.active||isCar(o))continue;
    if(o.path&&o.wait<=0){
      const distance=Math.round(o.distance)+o.index*2,phase=distance%8,cycle=Math.floor(distance/4),feet=walkingFeet(phase,o.travelY||0),frame=humanFrames(o.spriteType,o.travelY||0)[o.frame];
      ...
      if(frame[foot.y][foot.x]!=='O')issues.add('Missing planted sole');if(frame[foot.y][foot.x]!=='O'&&!window.firstMissing)window.firstMissing={o,phase,foot,pixel:frame[foot.y][foot.x]};
      const prior=plants.get(key(o));if(prior&&prior.cycle===cycle&&prior.axis===o.travelY&&prior.direction===o.direction){plantChecks++;if(x!==prior.x||y!==prior.y){issues.add('Planted foot slides');if(!window.firstSlide)window.firstSlide={o,prior,x,y,phase};}}
 ...
 return {issues:[...issues],frames:pixelWorld.inspect().frameCount,plantChecks,firstMissing:window.firstMissing,firstSlide:window.firstSlide};
