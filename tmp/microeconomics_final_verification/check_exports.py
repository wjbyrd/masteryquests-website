...
for area,g in groups.items():
    rows=e.make_rows(g,root,library)
    file=root/'faculty_exports'/ (e.DISCIPLINES[area][1]+'.csv')
    with file.open(encoding='utf-8-sig',newline='') as f: got=list(csv.DictReader(f))
    errors=[(r['question_id'],k) for r,s in zip(rows,got) for k in set(r)|set(s) if r.get(k,'')!=s.get(k,'')]
    companion[area]={'count':len(g),'rows':len(got),'unique':len({r['question_id'] for r in got}),'mismatches':errors,'ids':list(g)}
(root/'tmp/microeconomics_final_verification/companion_exports.json').write_text(json.dumps(companion),encoding='utf-8')
...
result={'records':len(group),'ids':list(group),'csv_rows':len(actual),'csv_unique_ids':len(set(r['question_id'] for r in actual)),
 'csv_columns':len(actual[0]),'csv_mismatches':csv_errors,'structure_errors':structure_errors,'pdf_pages':len(texts), 'pdf_blank_pages':[i+1 for i,t in enumerate(texts) if not t.strip()], 'pdf_id_order_matches':re.findall(r'Question ID:\s*([^\s]+)', '\n'.join(texts))==[r['question_id'] for r in expected],
...
(root/'tmp/microeconomics_final_verification/export_checks.json').write_text(json.dumps(result,indent=2),encoding='utf-8')
