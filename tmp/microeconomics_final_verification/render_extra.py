...
assets=json.loads((W/'extra_assets.json').read_text(encoding='utf-8'))
manifest=[]
for start in range(0,len(assets),6):
 ...
 sheet.save(W/f'extra_{start//6+1:02}.png')
(W/'extra_image_manifest.json').write_text(json.dumps(manifest,indent=2),encoding='utf-8')
