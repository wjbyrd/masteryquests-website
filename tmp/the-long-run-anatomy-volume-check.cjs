  ...
  for(let tick=0;tick<ticks;tick++){stepWorld(1);const current=pixelWorld.inspect().objects;
   ...
   for(const o of current){const previous=prior.find(p=>key(p)===key(o));if(o.path&&o.active&&previous?.active&&Math.abs(o.x-previous.x)>2)issues.add('Motion jump');if(!o.active||isCar(o))continue;
    if(o.path&&o.wait<=0){
      ...
      const foot=feet.planted,offset=SPRITE_PLACEMENT[o.spriteType],x=Math.round(o.x)+offset.x+(o.direction<0?21-(foot.x+2):foot.x+2),y=Math.round(o.y)+offset.y+foot.y;
    ...
    const off=SPRITE_PLACEMENT[o.spriteType],frame=personFrames(o.spriteType,o.travelY||0)[o.idle?SPRITE_WALK.idleFrame:o.frame];frame.forEach((row,y)=>[...row].forEach((c,x)=>{if(c===' ')return;const px=Math.round(o.x)+off.x+(o.direction<0?row.length-1-x:x),py=Math.round(o.y)+off.y+y;if(y>=24&&tables.some(t=>px>=t.x&&px<t.x+t.w&&py>=t.y&&py<t.y+t.h))issues.add('Table contact');if(y===frame.length-1&&obstacles.some(t=>px>=t.x&&px<t.x+t.w&&py>=t.y&&py<t.y+t.h))issues.add('Feet in furniture');}));}
   prior=current;
  }
 }
...
for(const [label,ids] of Object.entries({baseline:[],busy:['confidence','fiscal-expand','rate-cut'],recession:['energy','fiscal-contract','rate-hike']})){
 ...
 await page.evaluate(()=>stepWorld(30));await page.locator('.world-window').screenshot({path:`tmp/long-run-anatomy-volume-${label}.png`});
}
