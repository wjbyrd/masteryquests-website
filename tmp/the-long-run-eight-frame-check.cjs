...
for(const [name,p] of [['before',before],['after',page]]){
 ...
 });fs.writeFileSync(`tmp/long-run-eight-frame-actors-${name}.png`,Buffer.from(sheet,'base64'));
}
...
for(const [label,ids] of Object.entries({baseline:[],busy:['confidence','fiscal-expand','rate-cut'],recession:['energy','fiscal-contract','rate-hike']})){
 ...
 await page.evaluate(()=>stepWorld(30));await page.locator('.world-window').screenshot({path:`tmp/long-run-eight-frame-${label}.png`});
}
