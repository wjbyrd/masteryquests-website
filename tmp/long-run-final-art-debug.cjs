...
assert.equal(section(now,'function walkingPhase(','function walkingFeet('),section(old,'function walkingPhase(','function walkingFeet('));
...
await page.addInitScript(()=>{let next=0,t=1000;const q=new Map();window.requestAnimationFrame=cb=>{q.set(++next,cb);return next};window.cancelAnimationFrame=id=>q.delete(id);window.stepWorld=count=>{for(let i=0;i<count;i++){t+=1000/60;const jobs=[...q.values()];q.clear();jobs.forEach(cb=>cb(t));}};});
...
for(const [name,p] of [['before',before],['after',page]]){
 const sheet=await p.evaluate(()=>{
  const c=document.createElement('canvas');c.width=1520;c.height=960;const ctx=c.getContext('2d');ctx.fillStyle='#b9cfcd';ctx.fillRect(0,0,c.width,c.height);ctx.font='16px system-ui';
  ['pedestrian','shopper','factoryWorker','worker','resident','compact','sedan','truck'].forEach((type,i)=>{ctx.fillStyle='#192e40';ctx.fillText(type,8,i*120+20);sprites[type].frames.slice(0,SPRITE_WALK.idleFrame+1).forEach((frame,f)=>{frame.forEach((row,y)=>[...row].forEach((v,x)=>{if(v!==' '){ctx.fillStyle=PIXEL_PALETTE[v];ctx.fillRect(160+f*150+x*3,i*120+12+y*3,3,3)}}));});});return c.toDataURL().split(',')[1];
 });fs.writeFileSync(`tmp/long-run-final-art-actors-${name}.png`,Buffer.from(sheet,'base64'));
}
const dimensions=await page.evaluate(()=>Object.fromEntries(Object.entries(sprites).filter(([type])=>SPRITE_PLACEMENT[type]).map(([type,s])=>[type,[s.frames[0][0].length,s.frames[0].length]])));assert.deepEqual(dimensions.compact,[48,30]);assert.deepEqual(dimensions.sedan,[54,30]);assert.deepEqual(dimensions.truck,[58,31]);assert.deepEqual(dimensions.pedestrian,[20,30]);
const check=await page.evaluate(()=>{
 const issues=new Set(),plants=new Map();let plantChecks=0;const frameCache=new Map();const personFrames=(type,axis)=>{const key=type+axis;if(!frameCache.has(key))frameCache.set(key,humanFrames(type,axis));return frameCache.get(key)};const isCar=o=>['compact','sedan','truck'].includes(o.spriteType),key=o=>`${isCar(o)?'car':'human'}/${o.path}/${o.index}`;
 ...
 for(const [type,sprite] of Object.entries(sprites)){if(!SPRITE_PLACEMENT[type])continue;const h=sprite.frames[0].length,w=sprite.frames[0][0].length;for(const frame of sprite.frames){if(frame.length!==h||frame.some(row=>row.length!==w))issues.add('Frame size jitter');for(const row of frame)for(const c of row)if(c!==' '&&!(c in PIXEL_PALETTE))issues.add('Unknown color');}if(!['compact','sedan','truck'].includes(type)&&new Set(sprite.frames.slice(0,8).map(f=>f.join())).size!==8)issues.add('Indistinct walk poses');}
 function inspect(ticks){let prior=pixelWorld.inspect().objects;
  for(let tick=0;tick<ticks;tick++){stepWorld(1);const current=pixelWorld.inspect().objects;
   for(const lane of ['upperRoad','lowerRoad']){const cars=current.filter(o=>isCar(o)&&o.active&&o.path===lane).sort((a,b)=>a.x-b.x);for(let i=1;i<cars.length;i++)if(cars[i].x-cars[i-1].x<sprites[cars[i-1].spriteType].frames[0][0].length+6)issues.add('Car crowding');for(const car of cars){if(car.y!==(lane==='upperRoad'?197:218))issues.add('Lane drift');const y=vehicleDrawY(car.spriteType,car.path),contact=y+VEHICLE_WHEEL_CONTACT[car.spriteType];if(contact!==VEHICLE_ROAD_BASELINES[car.path]||contact-7<196||contact>=238)issues.add('Wheel contact off road');if(lane==='upperRoad'&&y>=196)issues.add('Roof trapped in road band');}}
   for(const o of current){const previous=prior.find(p=>key(p)===key(o));if(o.path&&o.active&&previous?.active&&Math.abs(o.x-previous.x)>2)issues.add('Motion jump');if(!o.active||isCar(o))continue;
    if(o.path&&o.wait<=0){
      const progress=(o.travelY?Math.round(o.y)*o.travelY:Math.round(o.x)*o.direction)+o.index*4,phase=walkingPhase(o),cycle=Math.floor(progress/8),feet=walkingFeet(phase,o.travelY||0),frame=personFrames(o.spriteType,o.travelY||0)[o.frame];
      const foot=feet.planted,offset=SPRITE_PLACEMENT[o.spriteType],x=Math.round(o.x)+offset.x+(o.direction<0?19-(foot.x+2):foot.x+2),y=Math.round(o.y)+offset.y+foot.y;
      if(frame[foot.y][foot.x+2]!=='O'){issues.add('Missing planted sole');if(!window.firstMissing)window.firstMissing={o,phase,foot,pixel:frame[foot.y][foot.x+2]};}
    ...
    const off=SPRITE_PLACEMENT[o.spriteType],frame=personFrames(o.spriteType,o.travelY||0)[o.idle?SPRITE_WALK.idleFrame:o.frame];frame.forEach((row,y)=>[...row].forEach((c,x)=>{if(c===' ')return;const px=Math.round(o.x)+off.x+(o.direction<0?row.length-1-x:x),py=Math.round(o.y)+off.y+y;if(y>=22&&tables.some(t=>px>=t.x&&px<t.x+t.w&&py>=t.y&&py<t.y+t.h))issues.add('Table contact');if(y===frame.length-1&&obstacles.some(t=>px>=t.x&&px<t.x+t.w&&py>=t.y&&py<t.y+t.h))issues.add('Feet in furniture');}));}
   prior=current;
  }
 }
 inspect(1920);for(const id of ['confidence','fiscal-expand','rate-cut','housing-boom','support']){const before=pixelWorld.inspect().objects;stepEconomy(id);renderWorld();const after=pixelWorld.inspect().objects;for(const o of before.filter(o=>o.path)){const next=after.find(n=>key(n)===key(o));if(next&&next.distance!==o.distance)issues.add('Year teleport');}inspect(1920);}
 return {issues:[...issues],frames:pixelWorld.inspect().frameCount,plantChecks,firstMissing:window.firstMissing};
});console.log(JSON.stringify(check));assert.deepEqual(check.issues,[]);assert.ok(check.plantChecks>10000);console.log('PASS: 192 seconds of simulated motion, lane clearances, sprite bounds/colors, furniture, wraparound and year continuity.',check);
const timing=await page.evaluate(()=>{economy=initialEconomy();recordYear();render();const person=()=>pixelWorld.inspect().objects.find(o=>o.path==='sidewalk');const start=person(),t=pixelWorld.inspect().time,frames=pixelWorld.inspect().frameCount;stepWorld(600);const end=person(),elapsed=pixelWorld.inspect().time-t;return {speed:(end.distance-start.distance)/elapsed,fps:(pixelWorld.inspect().frameCount-frames)/elapsed,crossingSeconds:368/PIXEL_TUNING.walkSpeed,gaitPixels:PIXEL_POLISH.walkCyclePixels,poseSeconds:2/PIXEL_TUNING.walkSpeed};});assert.ok(Math.abs(timing.speed-15)<.001);assert.ok(Math.abs(timing.fps-30)<.1);assert.ok(timing.crossingSeconds>20&&timing.crossingSeconds<30);assert.equal(timing.gaitPixels,16);console.log('PASS: measured walking speed/cadence',timing);
for(const [label,ids] of Object.entries({baseline:[],busy:['confidence','fiscal-expand','rate-cut'],recession:['energy','fiscal-contract','rate-hike']})){
 ...
 const oldPalette=await before.evaluate(()=>({...TOWN_PALETTE}));const background=await page.evaluate(oldPalette=>{const saved={...TOWN_PALETTE};Object.assign(TOWN_PALETTE,oldPalette,{cloudShade:oldPalette.cloud});renderWorld();const image=document.querySelector('#world').toDataURL();Object.assign(TOWN_PALETTE,saved);renderWorld();return image;},oldPalette);assert.equal(background,await before.locator('#world').evaluate(c=>c.toDataURL()));
 await page.evaluate(()=>stepWorld(30));await page.locator('.world-window').screenshot({path:`tmp/long-run-final-art-${label}.png`});
}
